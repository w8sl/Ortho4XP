...
/*   and tested against the size of the triangle.                            */
/*   Another modification is concerned with  element attributes which can    */
/*   can be of different types here for the same triangle. Not only dummy    */
...
struct mesh {
	...
	/* Variables used to allocate memory for triangles, subsegments,
	 * vertices,   */
	/*   viri (triangles being eaten), encroached segments, bad (skinny or
	 * too   */
	...
	int vertex2triindex; /* Index to find a triangle adjacent to a vertex.
			      */
	int highorderindex;  /* Index to find extra nodes for high-order
				elements. */
	...
	long
	    counterclockcount; /* Number of counterclockwise tests performed. */
	long orient3dcount;    /* Number of 3D orientation tests performed. */
	long hyperbolacount; /* Number of right-of-hyperbola tests performed. */
	long circumcentercount; /* Number of circumcenter calculations
				   performed. */
	...
	triangle *dummytri;
	triangle
	    *dummytribase; /* Keep base address so we can free() it later. */
...
struct behavior {
	...
	/*   steiner: maximum number of Steiner points, specified after -S
	 * switch.   */
	/*   incremental: -i switch.  sweepline: -F switch. */
	/*   dwyer: inverse of -l switch. */
	/*   splitseg: -s switch. */
	/*   conformdel: -D switch.  docheck: -C switch. */
	/*   quiet: -Q switch.  verbose: count of how often -V switch is
	 * selected.   */
	/*   usesegments: -p, -r, -q, or -c switch; determines whether segments
	 * are  */
	/*     used at all. */
	/*                                                                           */
	...
	char weight_filename[FILENAMESIZE];
	/* End of : Added for Triangle4XP */
#endif /* not TRILIBRARY */
...
#define decode(ptr, otri)                                                      \
	...
	(otri).tri = (triangle *)((INT_PTR)(ptr) ^ (INT_PTR)(otri).orient)

/* encode() compresses an oriented triangle into a single pointer.  It       */
/*   relies on the assumption that all triangles are aligned to four-byte    */
/*   boundaries, so the two least significant bits of (otri).tri are zero.   */

#define encode(otri) (triangle)((INT_PTR)(otri).tri | (INT_PTR)(otri).orient)
...
#define infect(otri)                                                           \
	(otri).tri[6] = (triangle)((INT_PTR)(otri).tri[6] | (INT_PTR)2l)
...
#define infected(otri) (((INT_PTR)(otri).tri[6] & (INT_PTR)2l) != 0l)
...
#define setelemattributeinit(otri, attnum, value)                              \
...
#define sdecode(sptr, osub)                                                    \
	...
	(osub).ss = (subseg *)((INT_PTR)(sptr) & ~(INT_PTR)3l)

/* sencode() compresses an oriented subsegment into a single pointer.  It    */
/*   relies on the assumption that all subsegments are aligned to two-byte   */
/*   boundaries, so the least significant bit of (osub).ss is zero.          */

#define sencode(osub) (subseg)((INT_PTR)(osub).ss | (INT_PTR)(osub).ssorient)
...
void syntax()
{
	...
	printf("    -p  Triangulates a Planar Straight Line Graph (.poly "
	       "file).\n");
	#ifndef CDT_ONLY
	printf("    -r  Refines a previously generated mesh.\n");
	printf("    -q  Quality mesh generation.  A minimum angle may be "
	       "specified.\n");
	printf("    -a  Applies a maximum triangle area constraint.\n");
	printf("    -u  Applies a user-defined triangle constraint.\n");
	#endif /* not CDT_ONLY */
	printf("    -A  Applies attributes to identify triangles in certain "
	       "regions.\n");
	printf("    -c  Encloses the convex hull with segments.\n");
	#ifndef CDT_ONLY
	printf("    -D  Conforming Delaunay:  all triangles are truly "
	       "Delaunay.\n");
	#endif /* not CDT_ONLY */
	       /*
		 printf("    -w  Weighted Delaunay triangulation.\n");
		 printf("    -W  Regular triangulation (lower hull of a height
		 field).\n");
	       */
	...
	printf("    -z  Numbers all items starting from zero (rather than "
	       "one).\n");
	...
	#endif /* not CDT_ONLY */
	#ifndef REDUCED
	printf("    -i  Uses incremental method, rather than "
	       "divide-and-conquer.\n");
	printf("    -F  Uses Fortune's sweepline algorithm, rather than "
	       "d-and-c.\n");
	#endif /* not REDUCED */
	printf(
	...
	#ifndef REDUCED
		#ifndef CDT_ONLY
	printf("    -s  Force segments into mesh by splitting (instead of "
	       "using CDT).\n");
		#endif /* not CDT_ONLY */
...
{
	printf("Triangle\n");
	printf("A Two-Dimensional Quality Mesh Generator and Delaunay "
	       "Triangulator.\n");
	printf("Version 1.6\n\n");
	printf("Copyright 1993, 1995, 1997, 1998, 2002, 2005 Jonathan Richard "
	       "Shewchuk\n");
	printf("2360 Woolsey #H / Berkeley, California 94705-1927\n");
	printf("Bugs/comments to jrs@cs.berkeley.edu\n");
	printf("Created as part of the Quake project (tools for earthquake "
	       "simulation).\n");
	printf("Supported in part by NSF Grant CMS-9318163 and an NSERC 1967 "
	       "Scholarship.\n");
	printf("There is no warranty whatsoever.  Use at your own risk.\n");
	#ifdef SINGLE
	printf("This executable is compiled for single precision "
	       "arithmetic.\n\n\n");
	#else  /* not SINGLE */
	printf("This executable is compiled for double precision "
	       "arithmetic.\n\n\n");
	#endif /* not SINGLE */
	printf("Triangle generates exact Delaunay triangulations, constrained "
	       "Delaunay\n");
	printf("triangulations, conforming Delaunay triangulations, Voronoi "
	       "diagrams, and\n");
	printf("high-quality triangular meshes.  The latter can be generated "
	       "with no small\n");
	printf("or large angles, and are thus suitable for finite element "
	       "analysis.  If no\n");
	printf("command line switch is specified, your .node input file is "
	       "read, and the\n");
	printf("Delaunay triangulation is returned in .node and .ele output "
	       "files.  The\n");
	printf("command syntax is:\n\n");
	printf("triangle [-prq__a__uAcDjevngBPNEIOXzo_YS__iFlsCQVh] "
	       "input_file\n\n");
	printf("Underscores indicate that numbers may optionally follow "
	       "certain switches.\n");
	printf("Do not leave any space between a switch and its numeric "
	       "parameter.\n");
	printf("input_file must be a file with extension .node, or extension "
	       ".poly if the\n");
	printf("-p switch is used.  If -r is used, you must supply .node and "
	       ".ele files,\n");
	printf("and possibly a .poly file and an .area file as well.  The "
	       "formats of these\n");
	printf("files are described below.\n\n");
	printf("Command Line Switches:\n\n");
	printf("    -p  Reads a Planar Straight Line Graph (.poly file), which "
	       "can specify\n");
	printf("        vertices, segments, holes, regional attributes, and "
	       "regional area\n");
	printf("        constraints.  Generates a constrained Delaunay "
	       "triangulation (CDT)\n");
	printf("        fitting the input; or, if -s, -q, -a, or -u is used, a "
	       "conforming\n");
	printf("        constrained Delaunay triangulation (CCDT).  If you "
	       "want a truly\n");
	printf("        Delaunay (not just constrained Delaunay) "
	       "triangulation, use -D as\n");
	printf("        well.  When -p is not used, Triangle reads a .node "
	       "file by default.\n");
	printf("    -r  Refines a previously generated mesh.  The mesh is read "
	       "from a .node\n");
	printf("        file and an .ele file.  If -p is also used, a .poly "
	       "file is read\n");
	printf("        and used to constrain segments in the mesh.  If -a is "
	       "also used\n");
	printf("        (with no number following), an .area file is read and "
	       "used to\n");
	printf("        impose area constraints on the mesh.  Further details "
	       "on refinement\n");
	printf("        appear below.\n");
	printf("    -q  Quality mesh generation by Delaunay refinement (a "
	       "hybrid of Paul\n");
	printf("        Chew's and Jim Ruppert's algorithms).  Adds vertices "
	       "to the mesh to\n");
	printf("        ensure that all angles are between 20 and 140 degrees. "
	       " An\n");
	printf("        alternative bound on the minimum angle, replacing 20 "
	       "degrees, may\n");
	printf("        be specified after the `q'.  The specified angle may "
	       "include a\n");
	printf("        decimal point, but not exponential notation.  Note "
	       "that a bound of\n");
	printf("        theta degrees on the smallest angle also implies a "
	       "bound of\n");
	printf("        (180 - 2 theta) on the largest angle.  If the minimum "
	       "angle is 28.6\n");
	printf("        degrees or smaller, Triangle is mathematically "
	       "guaranteed to\n");
	printf("        terminate (assuming infinite precision "
	       "arithmetic--Triangle may\n");
	printf("        fail to terminate if you run out of precision).  In "
	       "practice,\n");
	printf("        Triangle often succeeds for minimum angles up to 34 "
	       "degrees.  For\n");
	printf("        some meshes, however, you might need to reduce the "
	       "minimum angle to\n");
	printf("        avoid problems associated with insufficient "
	       "floating-point\n");
	printf("        precision.\n");
	printf("    -a  Imposes a maximum triangle area.  If a number follows "
	       "the `a', no\n");
	printf("        triangle is generated whose area is larger than that "
	       "number.  If no\n");
	printf("        number is specified, an .area file (if -r is used) or "
	       ".poly file\n");
	printf("        (if -r is not used) specifies a set of maximum area "
	       "constraints.\n");
	printf("        An .area file contains a separate area constraint for "
	       "each\n");
	printf("        triangle, and is useful for refining a finite element "
	       "mesh based on\n");
	printf("        a posteriori error estimates.  A .poly file can "
	       "optionally contain\n");
	printf("        an area constraint for each segment-bounded region, "
	       "thereby\n");
	printf("        controlling triangle densities in a first "
	       "triangulation of a PSLG.\n");
	printf("        You can impose both a fixed area constraint and a "
	       "varying area\n");
	printf("        constraint by invoking the -a switch twice, once with "
	       "and once\n");
	printf("        without a number following.  Each area specified may "
	       "include a\n");
	printf("        decimal point.\n");
	printf("    -u  Imposes a user-defined constraint on triangle size.  "
	       "There are two\n");
	printf("        ways to use this feature.  One is to edit the "
	       "triunsuitable()\n");
	printf("        procedure in triangle.c to encode any constraint you "
	       "like, then\n");
	printf("        recompile Triangle.  The other is to compile "
	       "triangle.c with the\n");
	printf("        EXTERNAL_TEST symbol set (compiler switch "
	       "-DEXTERNAL_TEST), then\n");
	printf("        link Triangle with a separate object file that "
	       "implements\n");
	printf("        triunsuitable().  In either case, the -u switch causes "
	       "the user-\n");
	printf("        defined test to be applied to every triangle.\n");
	printf("    -A  Assigns an additional floating-point attribute to each "
	       "triangle\n");
	printf("        that identifies what segment-bounded region each "
	       "triangle belongs\n");
	printf("        to.  Attributes are assigned to regions by the .poly "
	       "file.  If a\n");
	printf("        region is not explicitly marked by the .poly file, "
	       "triangles in\n");
	printf("        that region are assigned an attribute of zero.  The -A "
	       "switch has\n");
	printf("        an effect only when the -p switch is used and the -r "
	       "switch is not.\n");
	printf("    -c  Creates segments on the convex hull of the "
	       "triangulation.  If you\n");
	printf("        are triangulating a vertex set, this switch causes a "
	       ".poly file to\n");
	printf("        be written, containing all edges of the convex hull.  "
	       "If you are\n");
	printf("        triangulating a PSLG, this switch specifies that the "
	       "whole convex\n");
	printf("        hull of the PSLG should be triangulated, regardless of "
	       "what\n");
	printf("        segments the PSLG has.  If you do not use this switch "
	       "when\n");
	printf("        triangulating a PSLG, Triangle assumes that you have "
	       "identified the\n");
	printf("        region to be triangulated by surrounding it with "
	       "segments of the\n");
	printf("        input PSLG.  Beware:  if you are not careful, this "
	       "switch can cause\n");
	printf("        the introduction of an extremely thin angle between a "
	       "PSLG segment\n");
	printf("        and a convex hull segment, which can cause "
	       "overrefinement (and\n");
	printf("        possibly failure if Triangle runs out of precision).  "
	       "If you are\n");
	printf("        refining a mesh, the -c switch works differently:  it "
	       "causes a\n");
	printf("        .poly file to be written containing the boundary edges "
	       "of the mesh\n");
	printf("        (useful if no .poly file was read).\n");
	printf("    -D  Conforming Delaunay triangulation:  use this switch if "
	       "you want to\n");
	printf("        ensure that all the triangles in the mesh are "
	       "Delaunay, and not\n");
	printf("        merely constrained Delaunay; or if you want to ensure "
	       "that all the\n");
	printf("        Voronoi vertices lie within the triangulation.  (Some "
	       "finite volume\n");
	printf("        methods have this requirement.)  This switch invokes "
	       "Ruppert's\n");
	printf("        original algorithm, which splits every subsegment "
	       "whose diametral\n");
	printf("        circle is encroached.  It usually increases the number "
	       "of vertices\n");
	printf("        and triangles.\n");
	printf("    -j  Jettisons vertices that are not part of the final "
	       "triangulation\n");
	printf("        from the output .node file.  By default, Triangle "
	       "copies all\n");
	printf("        vertices in the input .node file to the output .node "
	       "file, in the\n");
	printf("        same order, so their indices do not change.  The -j "
	       "switch prevents\n");
	printf("        duplicated input vertices, or vertices `eaten' by "
	       "holes, from\n");
	printf("        appearing in the output .node file.  Thus, if two "
	       "input vertices\n");
	printf("        have exactly the same coordinates, only the first "
	       "appears in the\n");
	printf("        output.  If any vertices are jettisoned, the vertex "
	       "numbering in\n");
	printf("        the output .node file differs from that of the input "
	       ".node file.\n");
	printf("    -e  Outputs (to an .edge file) a list of edges of the "
	       "triangulation.\n");
	printf("    -v  Outputs the Voronoi diagram associated with the "
	       "triangulation.\n");
	printf("        Does not attempt to detect degeneracies, so some "
	       "Voronoi vertices\n");
	printf("        may be duplicated.  See the discussion of Voronoi "
	       "diagrams below.\n");
	printf("    -n  Outputs (to a .neigh file) a list of triangles "
	       "neighboring each\n");
	printf("        triangle.\n");
	printf("    -g  Outputs the mesh to an Object File Format (.off) file, "
	       "suitable for\n");
	printf(
	    "        viewing with the Geometry Center's Geomview package.\n");
	printf("    -B  No boundary markers in the output .node, .poly, and "
	       ".edge output\n");
	printf("        files.  See the detailed discussion of boundary "
	       "markers below.\n");
	printf("    -P  No output .poly file.  Saves disk space, but you lose "
	       "the ability\n");
	printf("        to maintain constraining segments on later refinements "
	       "of the mesh.\n");
	printf("    -N  No output .node file.\n");
	printf("    -E  No output .ele file.\n");
	printf("    -I  No iteration numbers.  Suppresses the output of .node "
	       "and .poly\n");
	printf("        files, so your input files won't be overwritten.  (If "
	       "your input is\n");
	printf("        a .poly file only, a .node file is written.)  Cannot "
	       "be used with\n");
	printf("        the -r switch, because that would overwrite your input "
	       ".ele file.\n");
	printf("        Shouldn't be used with the -q, -a, -u, or -s switch if "
	       "you are\n");
	printf("        using a .node file for input, because no .node file is "
	       "written, so\n");
	printf("        there is no record of any added Steiner points.\n");
	printf("    -O  No holes.  Ignores the holes in the .poly file.\n");
	printf("    -X  No exact arithmetic.  Normally, Triangle uses exact "
	       "floating-point\n");
	printf("        arithmetic for certain tests if it thinks the inexact "
	       "tests are not\n");
	printf("        accurate enough.  Exact arithmetic ensures the "
	       "robustness of the\n");
	printf("        triangulation algorithms, despite floating-point "
	       "roundoff error.\n");
	printf("        Disabling exact arithmetic with the -X switch causes a "
	       "small\n");
	printf("        improvement in speed and creates the possibility that "
	       "Triangle will\n");
	printf("        fail to produce a valid mesh.  Not recommended.\n");
	printf("    -z  Numbers all items starting from zero (rather than "
	       "one).  Note that\n");
	printf("        this switch is normally overridden by the value used "
	       "to number the\n");
	printf("        first vertex of the input .node or .poly file.  "
	       "However, this\n");
	printf("        switch is useful when calling Triangle from another "
	       "program.\n");
	printf("    -o2 Generates second-order subparametric elements with six "
	       "nodes each.\n");
	printf("    -Y  No new vertices on the boundary.  This switch is "
	       "useful when the\n");
	printf("        mesh boundary must be preserved so that it conforms to "
	       "some\n");
	printf("        adjacent mesh.  Be forewarned that you will probably "
	       "sacrifice much\n");
	printf("        of the quality of the mesh; Triangle will try, but the "
	       "resulting\n");
	printf("        mesh may contain poorly shaped triangles.  Works well "
	       "if all the\n");
	printf("        boundary vertices are closely spaced.  Specify this "
	       "switch twice\n");
	printf("        (`-YY') to prevent all segment splitting, including "
	       "internal\n");
	printf("        boundaries.\n");
	printf("    -S  Specifies the maximum number of Steiner points "
	       "(vertices that are\n");
	printf("        not in the input, but are added to meet the "
	       "constraints on minimum\n");
	printf("        angle and maximum area).  The default is to allow an "
	       "unlimited\n");
	printf("        number.  If you specify this switch with no number "
	       "after it,\n");
	printf("        the limit is set to zero.  Triangle always adds "
	       "vertices at segment\n");
	printf("        intersections, even if it needs to use more vertices "
	       "than the limit\n");
	printf("        you set.  When Triangle inserts segments by splitting "
	       "(-s), it\n");
	printf("        always adds enough vertices to ensure that all the "
	       "segments of the\n");
	printf(
	    "        PLSG are recovered, ignoring the limit if necessary.\n");
	printf("    -i  Uses an incremental rather than a divide-and-conquer "
	       "algorithm to\n");
	printf("        construct a Delaunay triangulation.  Try it if the "
	       "divide-and-\n");
	printf("        conquer algorithm fails.\n");
	printf("    -F  Uses Steven Fortune's sweepline algorithm to construct "
	       "a Delaunay\n");
	printf("        triangulation.  Warning:  does not use exact "
	       "arithmetic for all\n");
	printf("        calculations.  An exact result is not guaranteed.\n");
	printf("    -l  Uses only vertical cuts in the divide-and-conquer "
	       "algorithm.  By\n");
	printf("        default, Triangle alternates between vertical and "
	       "horizontal cuts,\n");
	printf("        which usually improve the speed except with vertex "
	       "sets that are\n");
	printf("        small or short and wide.  This switch is primarily of "
	       "theoretical\n");
	printf("        interest.\n");
	printf("    -s  Specifies that segments should be forced into the "
	       "triangulation by\n");
	printf("        recursively splitting them at their midpoints, rather "
	       "than by\n");
	printf("        generating a constrained Delaunay triangulation.  "
	       "Segment splitting\n");
	printf("        is true to Ruppert's original algorithm, but can "
	       "create needlessly\n");
	printf("        small triangles.  This switch is primarily of "
	       "theoretical interest.\n");
	printf("    -C  Check the consistency of the final mesh.  Uses exact "
	       "arithmetic for\n");
	printf("        checking, even if the -X switch is used.  Useful if "
	       "you suspect\n");
	printf("        Triangle is buggy.\n");
	printf("    -Q  Quiet:  Suppresses all explanation of what Triangle is "
	       "doing,\n");
	printf("        unless an error occurs.\n");
	printf("    -V  Verbose:  Gives detailed information about what "
	       "Triangle is doing.\n");
	printf("        Add more `V's for increasing amount of detail.  `-V' "
	       "is most\n");
	printf("        useful; itgives information on algorithmic progress "
	       "and much more\n");
	printf("        detailed statistics.  `-VV' gives vertex-by-vertex "
	       "details, and\n");
	printf("        prints so much that Triangle runs much more slowly.  "
	       "`-VVVV' gives\n");
	...
	printf("Definitions:\n");
	printf("\n");
	printf("  A Delaunay triangulation of a vertex set is a triangulation "
	       "whose\n");
	printf("  vertices are the vertex set, that covers the convex hull of "
	       "the vertex\n");
	printf("  set.  A Delaunay triangulation has the property that no "
	       "vertex lies\n");
	printf("  inside the circumscribing circle (circle that passes through "
	       "all three\n");
	printf("  vertices) of any triangle in the triangulation.\n\n");
	printf("  A Voronoi diagram of a vertex set is a subdivision of the "
	       "plane into\n");
	printf("  polygonal cells (some of which may be unbounded, meaning "
	       "infinitely\n");
	printf("  large), where each cell is the set of points in the plane "
	       "that are closer\n");
	printf("  to some input vertex than to any other input vertex.  The "
	       "Voronoi diagram\n");
	printf("  is a geometric dual of the Delaunay triangulation.\n\n");
	printf("  A Planar Straight Line Graph (PSLG) is a set of vertices and "
	       "segments.\n");
	printf("  Segments are simply edges, whose endpoints are all vertices "
	       "in the PSLG.\n");
	printf("  Segments may intersect each other only at their endpoints.  "
	       "The file\n");
	printf("  format for PSLGs (.poly files) is described below.\n\n");
	printf("  A constrained Delaunay triangulation (CDT) of a PSLG is "
	       "similar to a\n");
	printf("  Delaunay triangulation, but each PSLG segment is present as "
	       "a single edge\n");
	printf("  of the CDT.  (A constrained Delaunay triangulation is not "
	       "truly a\n");
	printf("  Delaunay triangulation, because some of its triangles might "
	       "not be\n");
	printf("  Delaunay.)  By definition, a CDT does not have any vertices "
	       "other than\n");
	printf("  those specified in the input PSLG.  Depending on context, a "
	       "CDT might\n");
	printf("  cover the convex hull of the PSLG, or it might cover only a "
	       "segment-\n");
	printf("  bounded region (e.g. a polygon).\n\n");
	printf("  A conforming Delaunay triangulation of a PSLG is a "
	       "triangulation in which\n");
	printf("  each triangle is truly Delaunay, and each PSLG segment is "
	       "represented by\n");
	printf("  a linear contiguous sequence of edges of the triangulation.  "
	       "New vertices\n");
	printf("  (not part of the PSLG) may appear, and each input segment "
	       "may have been\n");
	printf("  subdivided into shorter edges (subsegments) by these "
	       "additional vertices.\n");
	printf("  The new vertices are frequently necessary to maintain the "
	       "Delaunay\n");
	printf(
	    "  property while ensuring that every segment is represented.\n\n");
	printf("  A conforming constrained Delaunay triangulation (CCDT) of a "
	       "PSLG is a\n");
	printf("  triangulation of a PSLG whose triangles are constrained "
	       "Delaunay.  New\n");
	printf("  vertices may appear, and input segments may be subdivided "
	       "into\n");
	printf("  subsegments, but not to guarantee that segments are "
	       "respected; rather, to\n");
	printf("  improve the quality of the triangles.  The high-quality "
	       "meshes produced\n");
	printf("  by the -q switch are usually CCDTs, but can be made "
	       "conforming Delaunay\n");
	printf("  with the -D switch.\n\n");
	printf("File Formats:\n\n");
	printf("  All files may contain comments prefixed by the character "
	       "'#'.  Vertices,\n");
	printf("  triangles, edges, holes, and maximum area constraints must "
	       "be numbered\n");
	printf("  consecutively, starting from either 1 or 0.  Whichever you "
	       "choose, all\n");
	printf("  input files must be consistent; if the vertices are numbered "
	       "from 1, so\n");
	printf("  must be all other objects.  Triangle automatically detects "
	       "your choice\n");
	printf("  while reading the .node (or .poly) file.  (When calling "
	       "Triangle from\n");
	printf("  another program, use the -z switch if you wish to number "
	       "objects from\n");
	printf("  zero.)  Examples of these file formats are given below.\n\n");
	printf("  .node files:\n");
	printf("    First line:  <# of vertices> <dimension (must be 2)> <# of "
	       "attributes>\n");
	printf("                                           <# of boundary "
	       "markers (0 or 1)>\n");
	printf("    Remaining lines:  <vertex #> <x> <y> [attributes] "
	       "[boundary marker]\n");
	printf("\n");
	printf("    The attributes, which are typically floating-point values "
	       "of physical\n");
	printf("    quantities (such as mass or conductivity) associated with "
	       "the nodes of\n");
	printf("    a finite element mesh, are copied unchanged to the output "
	       "mesh.  If -q,\n");
	printf("    -a, -u, -D, or -s is selected, each new Steiner point "
	       "added to the mesh\n");
	printf(
	    "    has attributes assigned to it by linear interpolation.\n\n");
	printf("    If the fourth entry of the first line is `1', the last "
	       "column of the\n");
	printf("    remainder of the file is assumed to contain boundary "
	       "markers.  Boundary\n");
	printf("    markers are used to identify boundary vertices and "
	       "vertices resting on\n");
	printf("    PSLG segments; a complete description appears in a section "
	       "below.  The\n");
	printf("    .node file produced by Triangle contains boundary markers "
	       "in the last\n");
	printf("    column unless they are suppressed by the -B switch.\n\n");
	printf("  .ele files:\n");
	printf("    First line:  <# of triangles> <nodes per triangle> <# of "
	       "attributes>\n");
	printf("    Remaining lines:  <triangle #> <node> <node> <node> ... "
	       "[attributes]\n");
	printf("\n");
	printf("    Nodes are indices into the corresponding .node file.  The "
	       "first three\n");
	printf("    nodes are the corner vertices, and are listed in "
	       "counterclockwise order\n");
	printf("    around each triangle.  (The remaining nodes, if any, "
	       "depend on the type\n");
	printf("    of finite element used.)\n\n");
	printf("    The attributes are just like those of .node files.  "
	       "Because there is no\n");
	printf("    simple mapping from input to output triangles, Triangle "
	       "attempts to\n");
	printf("    interpolate attributes, and may cause a lot of diffusion "
	       "of attributes\n");
	printf("    among nearby triangles as the triangulation is refined.  "
	       "Attributes do\n");
	printf("    not diffuse across segments, so attributes used to "
	       "identify\n");
	printf("    segment-bounded regions remain intact.\n\n");
	printf("    In .ele files produced by Triangle, each triangular "
	       "element has three\n");
	printf("    nodes (vertices) unless the -o2 switch is used, in which "
	       "case\n");
	printf("    subparametric quadratic elements with six nodes each are "
	       "generated.\n");
	printf("    The first three nodes are the corners in counterclockwise "
	       "order, and\n");
	printf("    the fourth, fifth, and sixth nodes lie on the midpoints of "
	       "the edges\n");
	printf("    opposite the first, second, and third vertices, "
	       "respectively.\n");
	printf("\n");
	printf("  .poly files:\n");
	printf("    First line:  <# of vertices> <dimension (must be 2)> <# of "
	...
	printf("                                           <# of boundary "
	       "markers (0 or 1)>\n");
	printf("    Following lines:  <vertex #> <x> <y> [attributes] "
	       "[boundary marker]\n");
	printf("    One line:  <# of segments> <# of boundary markers (0 or "
	       "1)>\n");
	printf("    Following lines:  <segment #> <endpoint> <endpoint> "
	       "[boundary marker]\n");
	printf("    One line:  <# of holes>\n");
	printf("    Following lines:  <hole #> <x> <y>\n");
	printf("    Optional line:  <# of regional attributes and/or area "
	       "constraints>\n");
	printf("    Optional following lines:  <region #> <x> <y> <attribute> "
	       "<max area>\n");
	printf("\n");
	printf("    A .poly file represents a PSLG, as well as some additional "
	       "information.\n");
	printf("    The first section lists all the vertices, and is identical "
	       "to the\n");
	printf("    format of .node files.  <# of vertices> may be set to zero "
	       "to indicate\n");
	printf("    that the vertices are listed in a separate .node file; "
	       ".poly files\n");
	printf("    produced by Triangle always have this format.  A vertex "
	       "set represented\n");
	printf("    this way has the advantage that it may easily be "
	       "triangulated with or\n");
	printf("    without segments (depending on whether the -p switch is "
	       "invoked).\n");
	printf("\n");
	printf("    The second section lists the segments.  Segments are edges "
	       "whose\n");
	printf("    presence in the triangulation is enforced.  (Depending on "
	       "the choice of\n");
	printf("    switches, segment might be subdivided into smaller edges). "
	       " Each\n");
	printf("    segment is specified by listing the indices of its two "
	       "endpoints.  This\n");
	printf("    means that you must include its endpoints in the vertex "
	       "list.  Each\n");
	printf("    segment, like each point, may have a boundary marker.\n\n");
	printf("    If -q, -a, -u, and -s are not selected, Triangle produces "
	       "a constrained\n");
	printf("    Delaunay triangulation (CDT), in which each segment "
	       "appears as a single\n");
	printf("    edge in the triangulation.  If -q, -a, -u, or -s is "
	       "selected, Triangle\n");
	printf("    produces a conforming constrained Delaunay triangulation "
	       "(CCDT), in\n");
	printf("    which segments may be subdivided into smaller edges.  If "
	       "-D is\n");
	printf("    selected, Triangle produces a conforming Delaunay "
	       "triangulation, so\n");
	printf("    that every triangle is Delaunay, and not just constrained "
	       "Delaunay.\n");
	printf("\n");
	printf("    The third section lists holes (and concavities, if -c is "
	       "selected) in\n");
	printf("    the triangulation.  Holes are specified by identifying a "
	       "point inside\n");
	printf("    each hole.  After the triangulation is formed, Triangle "
	       "creates holes\n");
	printf("    by eating triangles, spreading out from each hole point "
	       "until its\n");
	printf("    progress is blocked by segments in the PSLG.  You must be "
	       "careful to\n");
	printf("    enclose each hole in segments, or your whole triangulation "
	       "might be\n");
	printf("    eaten away.  If the two triangles abutting a segment are "
	       "eaten, the\n");
	printf("    segment itself is also eaten.  Do not place a hole "
	       "directly on a\n");
	printf("    segment; if you do, Triangle chooses one side of the "
	       "segment\n");
	printf("    arbitrarily.\n\n");
	printf("    The optional fourth section lists regional attributes (to "
	       "be assigned\n");
	printf("    to all triangles in a region) and regional constraints on "
	       "the maximum\n");
	printf("    triangle area.  Triangle reads this section only if the -A "
	       "switch is\n");
	printf("    used or the -a switch is used without a number following "
	       "it, and the -r\n");
	printf("    switch is not used.  Regional attributes and area "
	       "constraints are\n");
	printf("    propagated in the same manner as holes:  you specify a "
	       "point for each\n");
	printf("    attribute and/or constraint, and the attribute and/or "
	       "constraint\n");
	printf("    affects the whole region (bounded by segments) containing "
	       "the point.\n");
	printf("    If two values are written on a line after the x and y "
	       "coordinate, the\n");
	printf("    first such value is assumed to be a regional attribute "
	       "(but is only\n");
	printf("    applied if the -A switch is selected), and the second "
	       "value is assumed\n");
	printf("    to be a regional area constraint (but is only applied if "
	       "the -a switch\n");
	printf("    is selected).  You may specify just one value after the "
	       "coordinates,\n");
	printf("    which can serve as both an attribute and an area "
	       "constraint, depending\n");
	printf("    on the choice of switches.  If you are using the -A and -a "
	       "switches\n");
	printf("    simultaneously and wish to assign an attribute to some "
	       "region without\n");
	printf("    imposing an area constraint, use a negative maximum "
	       "area.\n\n");
	printf("    When a triangulation is created from a .poly file, you "
	       "must either\n");
	printf("    enclose the entire region to be triangulated in PSLG "
	       "segments, or\n");
	printf("    use the -c switch, which automatically creates extra "
	       "segments that\n");
	printf("    enclose the convex hull of the PSLG.  If you do not use "
	       "the -c switch,\n");
	printf("    Triangle eats all triangles that are not enclosed by "
	       "segments; if you\n");
	printf("    are not careful, your whole triangulation may be eaten "
	       "away.  If you do\n");
	printf("    use the -c switch, you can still produce concavities by "
	       "the appropriate\n");
	printf("    placement of holes just inside the boundary of the convex "
	       "hull.\n");
	printf("\n");
	printf("    An ideal PSLG has no intersecting segments, nor any "
	       "vertices that lie\n");
	printf("    upon segments (except, of course, the endpoints of each "
	       "segment).  You\n");
	printf("    aren't required to make your .poly files ideal, but you "
	       "should be aware\n");
	printf("    of what can go wrong.  Segment intersections are "
	       "relatively safe--\n");
	printf("    Triangle calculates the intersection points for you and "
	       "adds them to\n");
	printf("    the triangulation--as long as your machine's "
	       "floating-point precision\n");
	printf("    doesn't become a problem.  You are tempting the fates if "
	       "you have three\n");
	printf("    segments that cross at the same location, and expect "
	       "Triangle to figure\n");
	printf("    out where the intersection point is.  Thanks to "
	       "floating-point roundoff\n");
	printf("    error, Triangle will probably decide that the three "
	       "segments intersect\n");
	printf("    at three different points, and you will find a minuscule "
	       "triangle in\n");
	printf("    your output--unless Triangle tries to refine the tiny "
	       "triangle, uses\n");
	printf("    up the last bit of machine precision, and fails to "
	       "terminate at all.\n");
	printf("    You're better off putting the intersection point in the "
	       "input files,\n");
	printf("    and manually breaking up each segment into two.  "
	       "Similarly, if you\n");
	printf("    place a vertex at the middle of a segment, and hope that "
	       "Triangle will\n");
	printf("    break up the segment at that vertex, you might get lucky.  "
	       "On the other\n");
	printf("    hand, Triangle might decide that the vertex doesn't lie "
	       "precisely on\n");
	printf("    the segment, and you'll have a needle-sharp triangle in "
	       "your output--or\n");
	printf("    a lot of tiny triangles if you're generating a quality "
	       "mesh.\n");
	printf("\n");
	printf("    When Triangle reads a .poly file, it also writes a .poly "
	       "file, which\n");
	printf("    includes all the subsegments--the edges that are parts of "
	       "input\n");
	printf("    segments.  If the -c switch is used, the output .poly file "
	       "also\n");
	printf("    includes all of the edges on the convex hull.  Hence, the "
	       "output .poly\n");
	printf("    file is useful for finding edges associated with input "
	       "segments and for\n");
	printf("    setting boundary conditions in finite element simulations. "
	       " Moreover,\n");
	printf("    you will need the output .poly file if you plan to refine "
	       "the output\n");
	printf("    mesh, and don't want segments to be missing in later "
	       "triangulations.\n");
	printf("\n");
	printf("  .area files:\n");
	...
	printf("\n");
	printf("    An .area file associates with each triangle a maximum area "
	       "that is used\n");
	printf("    for mesh refinement.  As with other file formats, every "
	       "triangle must\n");
	printf("    be represented, and the triangles must be numbered "
	       "consecutively.  A\n");
	printf("    triangle may be left unconstrained by assigning it a "
	       "negative maximum\n");
	printf("    area.\n\n");
	printf("  .edge files:\n");
	printf(
	    "    First line:  <# of edges> <# of boundary markers (0 or 1)>\n");
	printf("    Following lines:  <edge #> <endpoint> <endpoint> [boundary "
	       "marker]\n");
	printf("\n");
	printf("    Endpoints are indices into the corresponding .node file.  "
	       "Triangle can\n");
	printf("    produce .edge files (use the -e switch), but cannot read "
	       "them.  The\n");
	printf("    optional column of boundary markers is suppressed by the "
	       "-B switch.\n");
	printf("\n");
	printf("    In Voronoi diagrams, one also finds a special kind of edge "
	       "that is an\n");
	printf("    infinite ray with only one endpoint.  For these edges, a "
	       "different\n");
	printf("    format is used:\n\n");
	printf(
	    "        <edge #> <endpoint> -1 <direction x> <direction y>\n\n");
	printf("    The `direction' is a floating-point vector that indicates "
	       "the direction\n");
	printf("    of the infinite ray.\n\n");
	printf("  .neigh files:\n");
	printf("    First line:  <# of triangles> <# of neighbors per triangle "
	       "(always 3)>\n");
	printf("    Following lines:  <triangle #> <neighbor> <neighbor> "
	       "<neighbor>\n");
	printf("\n");
	printf("    Neighbors are indices into the corresponding .ele file.  "
	       "An index of -1\n");
	printf("    indicates no neighbor (because the triangle is on an "
	       "exterior\n");
	printf("    boundary).  The first neighbor of triangle i is opposite "
	       "the first\n");
	printf("    corner of triangle i, and so on.\n\n");
	printf("    Triangle can produce .neigh files (use the -n switch), but "
	       "cannot read\n");
	printf("    them.\n\n");
	printf("Boundary Markers:\n\n");
	printf("  Boundary markers are tags used mainly to identify which "
	       "output vertices\n");
	printf("  and edges are associated with which PSLG segment, and to "
	       "identify which\n");
	printf("  vertices and edges occur on a boundary of the triangulation. "
	       " A common\n");
	printf("  use is to determine where boundary conditions should be "
	       "applied to a\n");
	printf("  finite element mesh.  You can prevent boundary markers from "
	       "being written\n");
	printf("  into files produced by Triangle by using the -B switch.\n\n");
	printf("  The boundary marker associated with each segment in an "
	       "output .poly file\n");
	printf(
	    "  and each edge in an output .edge file is chosen as follows:\n");
	printf("    - If an output edge is part or all of a PSLG segment with "
	       "a nonzero\n");
	printf("      boundary marker, then the edge is assigned the same "
	       "marker.\n");
	printf("    - Otherwise, if the edge lies on a boundary of the "
	       "triangulation\n");
	printf("      (even the boundary of a hole), then the edge is assigned "
	       "the marker\n");
	printf("      one (1).\n");
	printf("    - Otherwise, the edge is assigned the marker zero (0).\n");
	printf("  The boundary marker associated with each vertex in an output "
	       ".node file\n");
	printf("  is chosen as follows:\n");
	printf("    - If a vertex is assigned a nonzero boundary marker in the "
	       "input file,\n");
	printf("      then it is assigned the same marker in the output .node "
	       "file.\n");
	printf("    - Otherwise, if the vertex lies on a PSLG segment (even if "
	       "it is an\n");
	printf("      endpoint of the segment) with a nonzero boundary marker, "
	       "then the\n");
	printf("      vertex is assigned the same marker.  If the vertex lies "
	       "on several\n");
	printf(
	    "      such segments, one of the markers is chosen arbitrarily.\n");
	printf("    - Otherwise, if the vertex occurs on a boundary of the "
	       "triangulation,\n");
	printf("      then the vertex is assigned the marker one (1).\n");
	printf(
	    "    - Otherwise, the vertex is assigned the marker zero (0).\n");
	printf("\n");
	printf("  If you want Triangle to determine for you which vertices and "
	       "edges are on\n");
	printf("  the boundary, assign them the boundary marker zero (or use "
	       "no markers at\n");
	printf("  all) in your input files.  In the output files, all boundary "
	       "vertices,\n");
	printf("  edges, and segments will be assigned the value one.\n\n");
	printf("Triangulation Iteration Numbers:\n\n");
	printf("  Because Triangle can read and refine its own triangulations, "
	       "input\n");
	printf("  and output files have iteration numbers.  For instance, "
	       "Triangle might\n");
	printf("  read the files mesh.3.node, mesh.3.ele, and mesh.3.poly, "
	       "refine the\n");
	printf("  triangulation, and output the files mesh.4.node, mesh.4.ele, "
	       "and\n");
	printf("  mesh.4.poly.  Files with no iteration number are treated as "
	       "if\n");
	printf("  their iteration number is zero; hence, Triangle might read "
	       "the file\n");
	printf("  points.node, triangulate it, and produce the files "
	       "points.1.node and\n");
	printf("  points.1.ele.\n\n");
	printf("  Iteration numbers allow you to create a sequence of "
	       "successively finer\n");
	printf("  meshes suitable for multigrid methods.  They also allow you "
	       "to produce a\n");
	printf("  sequence of meshes using error estimate-driven mesh "
	       "refinement.\n");
	printf("\n");
	printf("  If you're not using refinement or quality meshing, and you "
	       "don't like\n");
	printf("  iteration numbers, use the -I switch to disable them.  This "
	       "switch also\n");
	printf("  disables output of .node and .poly files to prevent your "
	       "input files from\n");
	printf("  being overwritten.  (If the input is a .poly file that "
	       "contains its own\n");
	printf("  points, a .node file is written.  This can be quite "
	       "convenient for\n");
	printf("  computing CDTs or quality meshes.)\n\n");
	printf("Examples of How to Use Triangle:\n\n");
	printf("  `triangle dots' reads vertices from dots.node, and writes "
	       "their Delaunay\n");
	printf("  triangulation to dots.1.node and dots.1.ele.  (dots.1.node "
	       "is identical\n");
	printf("  to dots.node.)  `triangle -I dots' writes the triangulation "
	       "to dots.ele\n");
	printf("  instead.  (No additional .node file is needed, so none is "
	       "written.)\n");
	printf("\n");
	printf("  `triangle -pe object.1' reads a PSLG from object.1.poly (and "
	       "possibly\n");
	printf("  object.1.node, if the vertices are omitted from "
	       "object.1.poly) and writes\n");
	printf("  its constrained Delaunay triangulation to object.2.node and "
	       "object.2.ele.\n");
	printf("  The segments are copied to object.2.poly, and all edges are "
	       "written to\n");
	printf("  object.2.edge.\n\n");
	printf("  `triangle -pq31.5a.1 object' reads a PSLG from object.poly "
	       "(and possibly\n");
	printf("  object.node), generates a mesh whose angles are all between "
	       "31.5 and 117\n");
	printf("  degrees and whose triangles all have areas of 0.1 or less, "
	       "and writes the\n");
	printf("  mesh to object.1.node and object.1.ele.  Each segment may be "
	       "broken up\n");
	printf("  into multiple subsegments; these are written to "
	       "object.1.poly.\n");
	printf("\n");
	printf("  Here is a sample file `box.poly' describing a square with a "
	       "square hole:\n");
	printf("\n");
	printf("    # A box with eight vertices in 2D, no attributes, one "
	       "boundary marker.\n");
	printf("    8 2 0 1\n");
	...
	printf("     1   1.5 1.5\n");
	printf("\n");
	printf("  Note that some segments are missing from the outer square, "
	       "so you must\n");
	printf("  use the `-c' switch.  After `triangle -pqc box.poly', here "
	       "is the output\n");
	printf("  file `box.1.node', with twelve vertices.  The last four "
	       "vertices were\n");
	printf("  added to meet the angle constraint.  Vertices 1, 2, and 9 "
	       "have markers\n");
	printf("  from segment 1.  Vertices 6 and 8 have markers from segment "
	       "4.  All the\n");
	printf("  other vertices but 4 have been marked to indicate that they "
	       "lie on a\n");
	printf("  boundary.\n\n");
	printf("    12  2  0  1\n");
	printf("       1    0   0      5\n");
	...
	printf("      12     8   4  12\n");
	printf("    # Generated by triangle -pqc box.poly\n\n");
	printf("  Here is the output file `box.1.poly'.  Note that segments "
	       "have been added\n");
	printf("  to represent the convex hull, and some segments have been "
	       "subdivided by\n");
	printf("  newly added vertices.  Note also that <# of vertices> is set "
	       "to zero to\n");
	printf("  indicate that the vertices should be read from the .node "
	       "file.\n");
	printf("\n");
	printf("    0  2  0  1\n");
	printf("    12  1\n");
	...
	printf("\n");
	printf("  The -r switch causes a mesh (.node and .ele files) to be "
	       "read and\n");
	printf("  refined.  If the -p switch is also used, a .poly file is "
	       "read and used to\n");
	printf("  specify edges that are constrained and cannot be eliminated "
	       "(although\n");
	printf("  they can be subdivided into smaller edges) by the refinement "
	       "process.\n");
	printf("\n");
	printf("  When you refine a mesh, you generally want to impose tighter "
	       "constraints.\n");
	printf("  One way to accomplish this is to use -q with a larger angle, "
	       "or -a\n");
	printf("  followed by a smaller area than you used to generate the "
	       "mesh you are\n");
	printf("  refining.  Another way to do this is to create an .area "
	       "file, which\n");
	printf("  specifies a maximum area for each triangle, and use the -a "
	       "switch\n");
	printf("  (without a number following).  Each triangle's area "
	       "constraint is applied\n");
	printf("  to that triangle.  Area constraints tend to diffuse as the "
	       "mesh is\n");
	printf("  refined, so if there are large variations in area constraint "
	       "between\n");
	printf("  adjacent triangles, you may not get the results you want.  "
	       "In that case,\n");
	printf("  consider instead using the -u switch and writing a C "
	       "procedure that\n");
	printf("  determines which triangles are too large.\n\n");
	printf("  If you are refining a mesh composed of linear (three-node) "
	       "elements, the\n");
	printf("  output mesh contains all the nodes present in the input "
	       "mesh, in the same\n");
	printf("  order, with new nodes added at the end of the .node file.  "
	       "However, the\n");
	printf("  refinement is not hierarchical: there is no guarantee that "
	       "each output\n");
	printf("  element is contained in a single input element.  Often, an "
	       "output element\n");
	printf("  can overlap two or three input elements, and some input "
	       "edges are not\n");
	printf("  present in the output mesh.  Hence, a sequence of refined "
	       "meshes forms a\n");
	printf("  hierarchy of nodes, but not a hierarchy of elements.  If you "
	       "refine a\n");
	printf("  mesh of higher-order elements, the hierarchical property "
	       "applies only to\n");
	printf("  the nodes at the corners of an element; the midpoint nodes "
	       "on each edge\n");
	printf("  are discarded before the mesh is refined.\n\n");
	printf("  Maximum area constraints in .poly files operate differently "
	       "from those in\n");
	printf("  .area files.  A maximum area in a .poly file applies to the "
	       "whole\n");
	printf("  (segment-bounded) region in which a point falls, whereas a "
	       "maximum area\n");
	printf("  in an .area file applies to only one triangle.  Area "
	       "constraints in .poly\n");
	printf("  files are used only when a mesh is first generated, whereas "
	       "area\n");
	printf("  constraints in .area files are used only to refine an "
	       "existing mesh, and\n");
	printf("  are typically based on a posteriori error estimates "
	       "resulting from a\n");
	printf("  finite element simulation on that mesh.\n\n");
	printf("  `triangle -rq25 object.1' reads object.1.node and "
	       "object.1.ele, then\n");
	printf("  refines the triangulation to enforce a 25 degree minimum "
	       "angle, and then\n");
	printf("  writes the refined triangulation to object.2.node and "
	       "object.2.ele.\n");
	printf("\n");
	printf("  `triangle -rpaa6.2 z.3' reads z.3.node, z.3.ele, z.3.poly, "
	       "and z.3.area.\n");
	printf("  After reconstructing the mesh and its subsegments, Triangle "
	       "refines the\n");
	printf("  mesh so that no triangle has area greater than 6.2, and "
	       "furthermore the\n");
	printf("  triangles satisfy the maximum area constraints in z.3.area.  "
	       "No angle\n");
	printf("  bound is imposed at all.  The output is written to z.4.node, "
	       "z.4.ele, and\n");
	printf("  z.4.poly.\n\n");
	printf("  The sequence `triangle -qa1 x', `triangle -rqa.3 x.1', "
	       "`triangle -rqa.1\n");
	printf("  x.2' creates a sequence of successively finer meshes x.1, "
	       "x.2, and x.3,\n");
	printf("  suitable for multigrid.\n\n");
	printf("Convex Hulls and Mesh Boundaries:\n\n");
	printf("  If the input is a vertex set (not a PSLG), Triangle produces "
	       "its convex\n");
	printf("  hull as a by-product in the output .poly file if you use the "
	       "-c switch.\n");
	printf("  There are faster algorithms for finding a two-dimensional "
	       "convex hull\n");
	printf("  than triangulation, of course, but this one comes for "
	       "free.\n\n");
	printf("  If the input is an unconstrained mesh (you are using the -r "
	       "switch but\n");
	printf("  not the -p switch), Triangle produces a list of its boundary "
	       "edges\n");
	printf("  (including hole boundaries) as a by-product when you use the "
	       "-c switch.\n");
	printf("  If you also use the -p switch, the output .poly file "
	       "contains all the\n");
	printf("  segments from the input .poly file as well.\n\n");
	printf("Voronoi Diagrams:\n\n");
	printf("  The -v switch produces a Voronoi diagram, in files suffixed "
	       ".v.node and\n");
	printf("  .v.edge.  For example, `triangle -v points' reads "
	       "points.node, produces\n");
	printf("  its Delaunay triangulation in points.1.node and "
	       "points.1.ele, and\n");
	printf("  produces its Voronoi diagram in points.1.v.node and "
	       "points.1.v.edge.  The\n");
	printf("  .v.node file contains a list of all Voronoi vertices, and "
	       "the .v.edge\n");
	printf("  file contains a list of all Voronoi edges, some of which may "
	       "be infinite\n");
	printf("  rays.  (The choice of filenames makes it easy to run the set "
	       "of Voronoi\n");
	printf("  vertices through Triangle, if so desired.)\n\n");
	printf("  This implementation does not use exact arithmetic to compute "
	       "the Voronoi\n");
	printf("  vertices, and does not check whether neighboring vertices "
	       "are identical.\n");
	printf("  Be forewarned that if the Delaunay triangulation is "
	       "degenerate or\n");
	printf("  near-degenerate, the Voronoi diagram may have duplicate "
	       "vertices or\n");
	printf("  crossing edges.\n\n");
	printf("  The result is a valid Voronoi diagram only if Triangle's "
	       "output is a true\n");
	printf("  Delaunay triangulation.  The Voronoi output is usually "
	       "meaningless (and\n");
	printf("  may contain crossing edges and other pathology) if the "
	       "output is a CDT or\n");
	printf("  CCDT, or if it has holes or concavities.  If the "
	       "triangulated domain is\n");
	printf("  convex and has no holes, you can use -D switch to force "
	       "Triangle to\n");
	printf("  construct a conforming Delaunay triangulation instead of a "
	       "CCDT, so the\n");
	printf("  Voronoi diagram will be valid.\n\n");
	printf("Mesh Topology:\n\n");
	printf("  You may wish to know which triangles are adjacent to a "
	       "certain Delaunay\n");
	printf("  edge in an .edge file, which Voronoi cells are adjacent to a "
	       "certain\n");
	printf("  Voronoi edge in a .v.edge file, or which Voronoi cells are "
	       "adjacent to\n");
	printf("  each other.  All of this information can be found by "
	       "cross-referencing\n");
	printf("  output files with the recollection that the Delaunay "
	       "triangulation and\n");
	printf("  the Voronoi diagram are planar duals.\n\n");
	printf("  Specifically, edge i of an .edge file is the dual of Voronoi "
	       "edge i of\n");
	printf("  the corresponding .v.edge file, and is rotated 90 degrees "
	       "counterclock-\n");
	printf("  wise from the Voronoi edge.  Triangle j of an .ele file is "
	       "the dual of\n");
	printf("  vertex j of the corresponding .v.node file.  Voronoi cell k "
	       "is the dual\n");
	printf("  of vertex k of the corresponding .node file.\n\n");
	printf("  Hence, to find the triangles adjacent to a Delaunay edge, "
	       "look at the\n");
	printf("  vertices of the corresponding Voronoi edge.  If the "
	       "endpoints of a\n");
	printf("  Voronoi edge are Voronoi vertices 2 and 6 respectively, then "
	       "triangles 2\n");
	printf("  and 6 adjoin the left and right sides of the corresponding "
	       "Delaunay edge,\n");
	printf("  respectively.  To find the Voronoi cells adjacent to a "
	       "Voronoi edge, look\n");
	printf("  at the endpoints of the corresponding Delaunay edge.  If the "
	       "endpoints of\n");
	printf("  a Delaunay edge are input vertices 7 and 12, then Voronoi "
	       "cells 7 and 12\n");
	printf("  adjoin the right and left sides of the corresponding Voronoi "
	       "edge,\n");
	printf("  respectively.  To find which Voronoi cells are adjacent to "
	       "each other,\n");
	printf("  just read the list of Delaunay edges.\n\n");
	printf("  Triangle does not write a list of the edges adjoining each "
	       "Voronoi cell,\n");
	printf("  but you can reconstructed it straightforwardly.  For "
	       "instance, to find\n");
	printf("  all the edges of Voronoi cell 1, search the output .edge "
	       "file for every\n");
	printf("  edge that has input vertex 1 as an endpoint.  The "
	       "corresponding dual\n");
	printf("  edges in the output .v.edge file form the boundary of "
	       "Voronoi cell 1.\n");
	printf("\n");
	printf("  For each Voronoi vertex, the .neigh file gives a list of the "
	       "three\n");
	printf("  Voronoi vertices attached to it.  You might find this more "
	       "convenient\n");
	printf("  than the .v.edge file.\n\n");
	printf("Quadratic Elements:\n\n");
	printf("  Triangle generates meshes with subparametric quadratic "
	       "elements if the\n");
	printf("  -o2 switch is specified.  Quadratic elements have six nodes "
	       "per element,\n");
	printf("  rather than three.  `Subparametric' means that the edges of "
	       "the triangles\n");
	printf("  are always straight, so that subparametric quadratic "
	       "elements are\n");
	printf("  geometrically identical to linear elements, even though they "
	       "can be used\n");
	printf("  with quadratic interpolating functions.  The three extra "
	       "nodes of an\n");
	printf("  element fall at the midpoints of the three edges, with the "
	       "fourth, fifth,\n");
	printf("  and sixth nodes appearing opposite the first, second, and "
	       "third corners\n");
	printf("  respectively.\n\n");
	printf("Domains with Small Angles:\n\n");
	printf("  If two input segments adjoin each other at a small angle, "
	       "clearly the -q\n");
	printf("  switch cannot remove the small angle.  Moreover, Triangle "
	       "may have no\n");
	printf("  choice but to generate additional triangles whose smallest "
	       "angles are\n");
	printf("  smaller than the specified bound.  However, these triangles "
	       "only appear\n");
	printf("  between input segments separated by small angles.  Moreover, "
	       "if you\n");
	printf("  request a minimum angle of theta degrees, Triangle will "
	       "generally produce\n");
	printf("  no angle larger than 180 - 2 theta, even if it is forced to "
	       "compromise on\n");
	printf("  the minimum angle.\n\n");
	printf("Statistics:\n\n");
	printf("  After generating a mesh, Triangle prints a count of entities "
	       "in the\n");
	printf("  output mesh, including the number of vertices, triangles, "
	       "edges, exterior\n");
	printf("  boundary edges (i.e. subsegments on the boundary of the "
	       "triangulation,\n");
	printf("  including hole boundaries), interior boundary edges (i.e. "
	       "subsegments of\n");
	printf("  input segments not on the boundary), and total subsegments.  "
	       "If you've\n");
	printf("  forgotten the statistics for an existing mesh, run Triangle "
	       "on that mesh\n");
	printf("  with the -rNEP switches to read the mesh and print the "
	       "statistics without\n");
	printf("  writing any files.  Use -rpNEP if you've got a .poly file "
	       "for the mesh.\n");
	printf("\n");
	printf("  The -V switch produces extended statistics, including a "
	       "rough estimate\n");
	printf("  of memory use, the number of calls to geometric predicates, "
	       "and\n");
	printf("  histograms of the angles and the aspect ratios of the "
	       "triangles in the\n");
	printf("  mesh.\n\n");
	printf("Exact Arithmetic:\n\n");
	printf("  Triangle uses adaptive exact arithmetic to perform what "
	       "computational\n");
	printf("  geometers call the `orientation' and `incircle' tests.  If "
	       "the floating-\n");
	printf("  point arithmetic of your machine conforms to the IEEE 754 "
	       "standard (as\n");
	printf("  most workstations do), and does not use extended precision "
	       "internal\n");
	printf("  floating-point registers, then your output is guaranteed to "
	       "be an\n");
	printf("  absolutely true Delaunay or constrained Delaunay "
	       "triangulation, roundoff\n");
	printf("  error notwithstanding.  The word `adaptive' implies that "
	       "these arithmetic\n");
	printf("  routines compute the result only to the precision necessary "
	       "to guarantee\n");
	printf("  correctness, so they are usually nearly as fast as their "
	       "approximate\n");
	printf("  counterparts.\n\n");
	printf("  May CPUs, including Intel x86 processors, have extended "
	       "precision\n");
	printf("  floating-point registers.  These must be reconfigured so "
	       "their precision\n");
	printf("  is reduced to memory precision.  Triangle does this if it is "
	       "compiled\n");
	printf("  correctly.  See the makefile for details.\n\n");
	printf("  The exact tests can be disabled with the -X switch.  On most "
	       "inputs, this\n");
	printf("  switch reduces the computation time by about eight "
	       "percent--it's not\n");
	printf("  worth the risk.  There are rare difficult inputs (having "
	       "many collinear\n");
	printf("  and cocircular vertices), however, for which the difference "
	       "in speed\n");
	printf("  could be a factor of two.  Be forewarned that these are "
	       "precisely the\n");
	printf("  inputs most likely to cause errors if you use the -X switch. "
	       " Hence, the\n");
	printf("  -X switch is not recommended.\n\n");
	printf("  Unfortunately, the exact tests don't solve every numerical "
	       "problem.\n");
	printf("  Exact arithmetic is not used to compute the positions of new "
	       "vertices,\n");
	printf("  because the bit complexity of vertex coordinates would grow "
	       "without\n");
	printf("  bound.  Hence, segment intersections aren't computed "
	       "exactly; in very\n");
	printf("  unusual cases, roundoff error in computing an intersection "
	       "point might\n");
	printf("  actually lead to an inverted triangle and an invalid "
	       "triangulation.\n");
	printf("  (This is one reason to specify your own intersection points "
	       "in your .poly\n");
	printf("  files.)  Similarly, exact arithmetic is not used to compute "
	       "the vertices\n");
	printf("  of the Voronoi diagram.\n\n");
	printf("  Another pair of problems not solved by the exact arithmetic "
	       "routines is\n");
	printf("  underflow and overflow.  If Triangle is compiled for double "
	       "precision\n");
	printf("  arithmetic, I believe that Triangle's geometric predicates "
	       "work correctly\n");
	printf("  if the exponent of every input coordinate falls in the range "
	       "[-148, 201].\n");
	printf("  Underflow can silently prevent the orientation and incircle "
	       "tests from\n");
	printf("  being performed exactly, while overflow typically causes a "
	       "floating\n");
	printf("  exception.\n\n");
	...
	printf("    If you're using a PSLG, you've probably failed to specify "
	       "a proper set\n");
	printf("    of bounding segments, or forgotten to use the -c switch.  "
	       "Or you may\n");
	printf("    have placed a hole badly, thereby eating all your "
	       "triangles.  To test\n");
	printf("    these possibilities, try again with the -c and -O "
	       "switches.\n");
	printf("    Alternatively, all your input vertices may be collinear, "
	       "in which case\n");
	printf("    you can hardly expect to triangulate them.\n\n");
	printf("  `Triangle doesn't terminate, or just crashes.'\n\n");
	printf("    Bad things can happen when triangles get so small that the "
	       "distance\n");
	printf("    between their vertices isn't much larger than the "
	       "precision of your\n");
	printf("    machine's arithmetic.  If you've compiled Triangle for "
	       "single-precision\n");
	printf("    arithmetic, you might do better by recompiling it for "
	       "double-precision.\n");
	printf("    Then again, you might just have to settle for more lenient "
	       "constraints\n");
	printf("    on the minimum angle and the maximum area than you had "
	       "planned.\n");
	printf("\n");
	printf("    You can minimize precision problems by ensuring that the "
	       "origin lies\n");
	printf("    inside your vertex set, or even inside the densest part of "
	       "your\n");
	printf("    mesh.  If you're triangulating an object whose "
	       "x-coordinates all fall\n");
	printf("    between 6247133 and 6247134, you're not leaving much "
	       "floating-point\n");
	printf("    precision for Triangle to work with.\n\n");
	printf("    Precision problems can occur covertly if the input PSLG "
	       "contains two\n");
	printf("    segments that meet (or intersect) at an extremely small "
	       "angle, or if\n");
	printf("    such an angle is introduced by the -c switch.  If you "
	       "don't realize\n");
	printf("    that a tiny angle is being formed, you might never "
	       "discover why\n");
	printf("    Triangle is crashing.  To check for this possibility, use "
	       "the -S switch\n");
	printf("    (with an appropriate limit on the number of Steiner "
	       "points, found by\n");
	printf("    trial-and-error) to stop Triangle early, and view the "
	       "output .poly file\n");
	printf("    with Show Me (described below).  Look carefully for "
	       "regions where dense\n");
	printf("    clusters of vertices are forming and for small angles "
	       "between segments.\n");
	printf("    Zoom in closely, as such segments might look like a single "
	       "segment from\n");
	printf("    a distance.\n\n");
	printf("    If some of the input values are too large, Triangle may "
	       "suffer a\n");
	printf("    floating exception due to overflow when attempting to "
	       "perform an\n");
	printf("    orientation or incircle test.  (Read the section on exact "
	       "arithmetic\n");
	printf("    above.)  Again, I recommend compiling Triangle for double "
	       "(rather\n");
	printf("    than single) precision arithmetic.\n\n");
	printf("    Unexpected problems can arise if you use quality meshing "
	       "(-q, -a, or\n");
	printf("    -u) with an input that is not segment-bounded--that is, if "
	       "your input\n");
	printf("    is a vertex set, or you're using the -c switch.  If the "
	       "convex hull of\n");
	printf("    your input vertices has collinear vertices on its "
	       "boundary, an input\n");
	printf("    vertex that you think lies on the convex hull might "
	       "actually lie just\n");
	printf("    inside the convex hull.  If so, the vertex and the nearby "
	       "convex hull\n");
	printf("    edge form an extremely thin triangle.  When Triangle tries "
	       "to refine\n");
	printf("    the mesh to enforce angle and area constraints, Triangle "
	       "might generate\n");
	printf("    extremely tiny triangles, or it might fail because of "
	       "insufficient\n");
	printf("    floating-point precision.\n\n");
	printf("  `The numbering of the output vertices doesn't match the "
	       "input vertices.'\n");
	printf("\n");
	printf("    You may have had duplicate input vertices, or you may have "
	       "eaten some\n");
	printf("    of your input vertices with a hole, or by placing them "
	       "outside the area\n");
	printf("    enclosed by segments.  In any case, you can solve the "
	       "problem by not\n");
	printf("    using the -j switch.\n\n");
	printf("  `Triangle executes without incident, but when I look at the "
	       "resulting\n");
	printf("  mesh, it has overlapping triangles or other geometric "
	       "inconsistencies.'\n");
	printf("\n");
	printf("    If you select the -X switch, Triangle occasionally makes "
	       "mistakes due\n");
	printf("    to floating-point roundoff error.  Although these errors "
	       "are rare,\n");
	printf("    don't use the -X switch.  If you still have problems, "
	       "please report the\n");
	printf("    bug.\n\n");
	printf("  `Triangle executes without incident, but when I look at the "
	       "resulting\n");
	...
	printf("\n");
	printf("    If your input is a PSLG (-p), you can only expect a "
	       "meaningful Voronoi\n");
	printf("    diagram if the domain you are triangulating is convex and "
	       "free of\n");
	printf("    holes, and you use the -D switch to construct a conforming "
	       "Delaunay\n");
	printf("    triangulation (instead of a CDT or CCDT).\n\n");
	printf("  Strange things can happen if you've taken liberties with "
	       "your PSLG.  Do\n");
	printf("  you have a vertex lying in the middle of a segment?  "
	       "Triangle sometimes\n");
	printf("  copes poorly with that sort of thing.  Do you want to lay "
	       "out a collinear\n");
	printf("  row of evenly spaced, segment-connected vertices?  Have you "
	       "simply\n");
	printf("  defined one long segment connecting the leftmost vertex to "
	       "the rightmost\n");

	printf("  vertex, and a bunch of vertices lying along it?  This method "
	       "occasionally\n");
	printf("  works, especially with horizontal and vertical lines, but "
	       "often it\n");
	printf("  doesn't, and you'll have to connect each adjacent pair of "
	       "vertices with a\n");
	printf("  separate segment.  If you don't like it, tough.\n\n");
	printf("  Furthermore, if you have segments that intersect other than "
	       "at their\n");
	printf("  endpoints, try not to let the intersections fall extremely "
	       "close to PSLG\n");
	printf("  vertices or each other.\n\n");
	printf("  If you have problems refining a triangulation not produced "
	       "by Triangle:\n");
	printf("  Are you sure the triangulation is geometrically valid?  Is "
	       "it formatted\n");
	printf("  correctly for Triangle?  Are the triangles all listed so the "
	       "first three\n");
	printf("  vertices are their corners in counterclockwise order?  Are "
	       "all of the\n");
	printf("  triangles constrained Delaunay?  Triangle's Delaunay "
	       "refinement algorithm\n");
	printf("  assumes that it starts with a CDT.\n\n");
	printf("Show Me:\n\n");
	printf("  Triangle comes with a separate program named `Show Me', "
	       "whose primary\n");
	printf("  purpose is to draw meshes on your screen or in PostScript.  "
	       "Its secondary\n");
	printf("  purpose is to check the validity of your input files, and do "
	       "so more\n");
	printf("  thoroughly than Triangle does.  Unlike Triangle, Show Me "
	       "requires that\n");
	printf("  you have the X Windows system.  Sorry, Microsoft Windows "
	       "users.\n");
	printf("\n");
	printf("Triangle on the Web:\n");
	printf("\n");
	printf("  To see an illustrated version of these instructions, check "
	       "out\n");
	printf("\n");
	...
	printf("A Brief Plea:\n");
	printf("\n");
	printf("  If you use Triangle, and especially if you use it to "
	       "accomplish real\n");
	printf("  work, I would like very much to hear from you.  A short "
	       "letter or email\n");
	printf("  (to jrs@cs.berkeley.edu) describing how you use Triangle "
	       "will mean a lot\n");
	printf("  to me.  The more people I know are using this program, the "
	       "more easily I\n");
	printf("  can justify spending time on improvements, which in turn "
	       "will benefit\n");
	printf("  you.  Also, I can put you on a list to receive email "
	       "whenever a new\n");
	printf("  version of Triangle is available.\n\n");
	printf("  If you use a mesh generated by Triangle in a publication, "
	       "please include\n");
	printf("  an acknowledgment as well.  And please spell Triangle with a "
	       "capital `T'!\n");
	printf("  If you want to include a citation, use `Jonathan Richard "
	       "Shewchuk,\n");
	printf("  ``Triangle: Engineering a 2D Quality Mesh Generator and "
	       "Delaunay\n");
	printf("  Triangulator,'' in Applied Computational Geometry:  Towards "
	       "Geometric\n");
	printf("  Engineering (Ming C. Lin and Dinesh Manocha, editors), "
	       "volume 1148 of\n");
	printf("  Lecture Notes in Computer Science, pages 203-222, "
	       "Springer-Verlag,\n");
	printf("  Berlin, May 1996.  (From the First ACM Workshop on Applied "
	       "Computational\n");
	printf("  Geometry.)'\n\n");
	printf("Research credit:\n\n");
	printf("  Of course, I can take credit for only a fraction of the "
	       "ideas that made\n");
	printf("  this mesh generator possible.  Triangle owes its existence "
	       "to the efforts\n");
	printf("  of many fine computational geometers and other researchers, "
	       "including\n");
	printf("  Marshall Bern, L. Paul Chew, Kenneth L. Clarkson, Boris "
	       "Delaunay, Rex A.\n");
	printf("  Dwyer, David Eppstein, Steven Fortune, Leonidas J. Guibas, "
	       "Donald E.\n");
	printf("  Knuth, Charles L. Lawson, Der-Tsai Lee, Gary L. Miller, "
	       "Ernst P. Mucke,\n");
	printf("  Steven E. Pav, Douglas M. Priest, Jim Ruppert, Isaac Saias, "
	       "Bruce J.\n");
	printf("  Schachter, Micha Sharir, Peter W. Shor, Daniel D. Sleator, "
	       "Jorge Stolfi,\n");
	printf("  Robert E. Tarjan, Alper Ungor, Christopher J. Van Wyk, Noel "
	       "J.\n");
	printf("  Walkington, and Binhai Zhu.  See the comments at the "
	       "beginning of the\n");
...
void internalerror()
{
	...
	printf("  Include the message above, your input data set, and the
...

{
			...
			for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
				...
				if (argv[i][j] == 'q') {
					...
					if (((argv[i][j + 1] >= '0') &&
						...
						workstring[k] = '\0';
						b->minangle = (REAL)strtod(
						    workstring, (char **)NULL);
					} else {
						b->minangle = 20.0;
					}
				}
	...
	if (b->weight_filename[0] == '\0') {
		syntax();
	}
	/* End of : Added for Triangle4XP */
	if (!strcmp(&b->innodefilename[strlen(b->innodefilename) - 5],
		    ".node")) {
	...
	#ifndef CDT_ONLY
	if (!strcmp(&b->innodefilename[strlen(b->innodefilename) - 4],
		    ".ele")) {
	...
	if (b->weighted && (b->poly || b->quality)) {
		b->weighted = 0;
		if (!b->quiet) {
			printf("Warning:  weighted triangulations (-w, -W) are "
			       "incompatible\n");
			printf("  with PSLGs (-p) and meshing (-q, -a, -u).  "
			       "Weights ignored.\n");
		}
	}
	if (b->jettison && b->nonodewritten && !b->quiet) {
		printf("Warning:  -j and -N switches are somewhat "
		       "incompatible.\n");
		printf("  If any vertices are jettisoned, you will need the "
		       "output\n");
...
/*****************************************************************************/
/*    Computing altitude and normals.                                        */
/*    BEWARE: (x,y) implicitly assumed to live in the DTM range.             */
/*                                                                           */
/*****************************************************************************/
...

{
	...
	pool->nextitem = (VOID *)(alignptr + (INT_PTR)pool->alignbytes -
...

{
	 ...
	 */
	/*   pointer (to point to the next block) are allocated, as well as
	 * space */
	...
	if (pool->deaditemstack != (VOID *)NULL) {
		...
		if (pool->unallocateditems == 0) {
			...
			if (*(pool->nowblock) == (VOID *)NULL) {
				/* Allocate a new block of items, pointed to by
				 * the previous block. */
				newblock = (VOID **)trimalloc(
				    pool->itemsperblock * pool->itembytes +
				    (int)sizeof(VOID *) + pool->alignbytes);
			...
			/* Align the item on an `alignbytes'-byte boundary. */
			pool->nextitem =
			    (VOID *)(alignptr + (INT_PTR)pool->alignbytes -
				     (alignptr % (INT_PTR)pool->alignbytes));
			/* There are lots of unallocated items left in this
			 * block. */
		...
		pool->nextitem =
		    (VOID *)((char *)pool->nextitem + pool->itembytes);
...

{
	...
	pool->pathitem = (VOID *)(alignptr + (INT_PTR)pool->alignbytes -
				  (alignptr % (INT_PTR)pool->alignbytes));
...

{
	...
	if (pool->pathitemsleft == 0) {
		...
		/* Find the first item in the block.  Increment by the size of
		 * (VOID *). */
		alignptr = (INT_PTR)(pool->pathblock + 1);
		/* Align with item on an `alignbytes'-byte boundary. */
		pool->pathitem =
...

{
	...
	m->dummytri =
	    (triangle *)(alignptr + (INT_PTR)m->triangles.alignbytes -
	...
	if (b->usesegments) {
		/* Set up `dummysub', the omnipresent subsegment pointed to by
		 * any */
		/*   triangle side or subsegment end that isn't attached to a
		 * real */
		/*   subsegment. */
		m->dummysubbase =
		...
		m->dummysub =
		    (subseg *)(alignptr + (INT_PTR)m->subsegs.alignbytes -
			       (alignptr % (INT_PTR)m->subsegs.alignbytes));
		/* Initialize the two adjoining subsegments to be the
		 * omnipresent      */
		/*   subsegment.  These will eventually be changed by various
		 * bonding  */
		/*   operations, but their values don't really matter, as long
		 * as they */
		...
		*(int *)(m->dummysub + 8) = 0;
		...
		/* Initialize the three adjoining subsegments of `dummytri' to
		 * be */
...

{
	 ...
	 */
	/*   followed by the vertex type.  Ensure the vertex marker is aligned
	 * to */
	...
	if (b->poly) {
		/* The index within each vertex at which a triangle pointer is
		 * found.  */
		/*   Ensure the pointer is aligned to a sizeof(triangle)-byte
		 * address. */
		m->vertex2triindex =
	...
	poolinit(&m->vertices, vertexsize, VERTEXPERBLOCK,
		 m->invertices > VERTEXPERBLOCK ? m->invertices
						: VERTEXPERBLOCK,
		 sizeof(REAL));
}
...

{
	...
	/*   pointers to other triangles, three pointers to corners, and
	 * possibly */
	...
	/* If triangle attributes or an area bound are needed, increase the
	 * number */
...

{
	...
	Two_Two_Diff(detleft, detlefttail, detright, detrighttail, B3, B[2],
		     B[1], B[0]);
	B[3] = B3;
...

{
	...
	REAL axtbct[16], aytbct[16], bxtcat[16], bytcat[16], cxtabt[16],
	    cytabt[16];
	...
	Two_Two_Diff(bdxcdy1, bdxcdy0, cdxbdy1, cdxbdy0, bc3, bc[2], bc[1],
		     bc[0]);
	bc[3] = bc3;
	...
	alen =
	    fast_expansion_sum_zeroelim(axxbclen, axxbc, ayybclen, ayybc, adet);

	Two_Product(cdx, ady, cdxady1, cdxady0);
	Two_Product(adx, cdy, adxcdy1, adxcdy0);
	Two_Two_Diff(cdxady1, cdxady0, adxcdy1, adxcdy0, ca3, ca[2], ca[1],
		     ca[0]);
	ca[3] = ca3;
	...
	blen =
	    fast_expansion_sum_zeroelim(bxxcalen, bxxca, byycalen, byyca, bdet);

	Two_Product(adx, bdy, adxbdy1, adxbdy0);
	Two_Product(bdx, ady, bdxady1, bdxady0);
	Two_Two_Diff(adxbdy1, adxbdy0, bdxady1, bdxady0, ab3, ab[2], ab[1],
		     ab[0]);
	ab[3] = ab3;
	...
	clen =
	    fast_expansion_sum_zeroelim(cxxablen, cxxab, cyyablen, cyyab, cdet);
	...
	if ((bdxtail != 0.0) || (bdytail != 0.0) || (cdxtail != 0.0) ||
		...
		Two_Two_Sum(adxadx1, adxadx0, adyady1, adyady0, aa3, aa[2],
			    aa[1], aa[0]);
		aa[3] = aa3;
	}
	if ((cdxtail != 0.0) || (cdytail != 0.0) || (adxtail != 0.0) ||
		...
		Two_Two_Sum(bdxbdx1, bdxbdx0, bdybdy1, bdybdy0, bb3, bb[2],
			    bb[1], bb[0]);
		bb[3] = bb3;
	}
	if ((adxtail != 0.0) || (adytail != 0.0) || (bdxtail != 0.0) ||
		...
		Two_Two_Sum(cdxcdx1, cdxcdx0, cdycdy1, cdycdy0, cc3, cc[2],
			    cc[1], cc[0]);
		cc[3] = cc3;
	}
	...
	if (adxtail != 0.0) {
		...
		temp16alen = scale_expansion_zeroelim(axtbclen, axtbc,
						      2.0 * adx, temp16a);
	...
	if (adytail != 0.0) {
		...
		temp16alen = scale_expansion_zeroelim(aytbclen, aytbc,
						      2.0 * ady, temp16a);
	...
	if (bdxtail != 0.0) {
		...
		temp16alen = scale_expansion_zeroelim(bxtcalen, bxtca,
						      2.0 * bdx, temp16a);
	...
	if (bdytail != 0.0) {
		...
		temp16alen = scale_expansion_zeroelim(bytcalen, bytca,
						      2.0 * bdy, temp16a);
	...
	if (cdxtail != 0.0) {
		...
		temp16alen = scale_expansion_zeroelim(cxtablen, cxtab,
						      2.0 * cdx, temp16a);
	...
	if (cdytail != 0.0) {
		...
		temp16alen = scale_expansion_zeroelim(cytablen, cytab,
						      2.0 * cdy, temp16a);
	...
	if ((adxtail != 0.0) || (adytail != 0.0)) {
		if ((bdxtail != 0.0) || (bdytail != 0.0) || (cdxtail != 0.0) ||
			...
			Two_Two_Diff(ti1, ti0, tj1, tj0, bctt3, bctt[2],
				     bctt[1], bctt[0]);
			bctt[3] = bctt3;
			bcttlen = 4;
		} else {
		...
		if (adxtail != 0.0) {
			temp16alen = scale_expansion_zeroelim(axtbclen, axtbc,
							      adxtail, temp16a);
			axtbctlen = scale_expansion_zeroelim(bctlen, bct,
							     adxtail, axtbct);
			temp32alen = scale_expansion_zeroelim(
			    axtbctlen, axtbct, 2.0 * adx, temp32a);
			temp48len = fast_expansion_sum_zeroelim(
			...
			if (bdytail != 0.0) {
				...
				temp16alen = scale_expansion_zeroelim(
				    temp8len, temp8, bdytail, temp16a);
				finlength = fast_expansion_sum_zeroelim(
				    finlength, finnow, temp16alen, temp16a,
				    finother);
				finswap = finnow;
				finnow = finother;
				finother = finswap;
			...
			if (cdytail != 0.0) {
				temp8len = scale_expansion_zeroelim(
				    4, bb, -adxtail, temp8);
				temp16alen = scale_expansion_zeroelim(
				    temp8len, temp8, cdytail, temp16a);
				finlength = fast_expansion_sum_zeroelim(
				    finlength, finnow, temp16alen, temp16a,
				    finother);
				finswap = finnow;
			...
			temp32alen = scale_expansion_zeroelim(axtbctlen, axtbct,
							      adxtail, temp32a);
			axtbcttlen = scale_expansion_zeroelim(bcttlen, bctt,
							      adxtail, axtbctt);
			temp16alen = scale_expansion_zeroelim(
			    axtbcttlen, axtbctt, 2.0 * adx, temp16a);
			temp16blen = scale_expansion_zeroelim(
			    axtbcttlen, axtbctt, adxtail, temp16b);
		...
		if (adytail != 0.0) {
			temp16alen = scale_expansion_zeroelim(aytbclen, aytbc,
							      adytail, temp16a);
			aytbctlen = scale_expansion_zeroelim(bctlen, bct,
							     adytail, aytbct);
			temp32alen = scale_expansion_zeroelim(
			    aytbctlen, aytbct, 2.0 * ady, temp32a);
			temp48len = fast_expansion_sum_zeroelim(
			    temp16alen, temp16a, temp32alen, temp32a, temp48);
			finlength = fast_expansion_sum_zeroelim(
			    finlength, finnow, temp48len, temp48, finother);
			...
			temp32alen = scale_expansion_zeroelim(aytbctlen, aytbct,
							      adytail, temp32a);
			aytbcttlen = scale_expansion_zeroelim(bcttlen, bctt,
							      adytail, aytbctt);
			temp16alen = scale_expansion_zeroelim(
			    aytbcttlen, aytbctt, 2.0 * ady, temp16a);
			temp16blen = scale_expansion_zeroelim(
			    aytbcttlen, aytbctt, adytail, temp16b);
			temp32blen = fast_expansion_sum_zeroelim(
			    temp16alen, temp16a, temp16blen, temp16b, temp32b);
			temp64len = fast_expansion_sum_zeroelim(
			    temp32alen, temp32a, temp32blen, temp32b, temp64);
	...
	if ((bdxtail != 0.0) || (bdytail != 0.0)) {
		if ((cdxtail != 0.0) || (cdytail != 0.0) || (adxtail != 0.0) ||
			...
			Two_Two_Diff(ti1, ti0, tj1, tj0, catt3, catt[2],
				     catt[1], catt[0]);
			catt[3] = catt3;
			cattlen = 4;
		} else {
		...
		if (bdxtail != 0.0) {
			temp16alen = scale_expansion_zeroelim(bxtcalen, bxtca,
							      bdxtail, temp16a);
			bxtcatlen = scale_expansion_zeroelim(catlen, cat,
							     bdxtail, bxtcat);
			temp32alen = scale_expansion_zeroelim(
			    bxtcatlen, bxtcat, 2.0 * bdx, temp32a);
			temp48len = fast_expansion_sum_zeroelim(
			    temp16alen, temp16a, temp32alen, temp32a, temp48);
			finlength = fast_expansion_sum_zeroelim(
			    finlength, finnow, temp48len, temp48, finother);
			...
			if (adytail != 0.0) {
				temp8len = scale_expansion_zeroelim(
				    4, cc, -bdxtail, temp8);
				temp16alen = scale_expansion_zeroelim(
				    temp8len, temp8, adytail, temp16a);
				finlength = fast_expansion_sum_zeroelim(
				    finlength, finnow, temp16alen, temp16a,
				    finother);
				finswap = finnow;
			...
			temp32alen = scale_expansion_zeroelim(bxtcatlen, bxtcat,
							      bdxtail, temp32a);
			bxtcattlen = scale_expansion_zeroelim(cattlen, catt,
							      bdxtail, bxtcatt);
			temp16alen = scale_expansion_zeroelim(
			    bxtcattlen, bxtcatt, 2.0 * bdx, temp16a);
			temp16blen = scale_expansion_zeroelim(
			    bxtcattlen, bxtcatt, bdxtail, temp16b);
			temp32blen = fast_expansion_sum_zeroelim(
			    temp16alen, temp16a, temp16blen, temp16b, temp32b);
			temp64len = fast_expansion_sum_zeroelim(
			    temp32alen, temp32a, temp32blen, temp32b, temp64);
		...
		if (bdytail != 0.0) {
			temp16alen = scale_expansion_zeroelim(bytcalen, bytca,
							      bdytail, temp16a);
			bytcatlen = scale_expansion_zeroelim(catlen, cat,
							     bdytail, bytcat);
			temp32alen = scale_expansion_zeroelim(
			    bytcatlen, bytcat, 2.0 * bdy, temp32a);
			temp48len = fast_expansion_sum_zeroelim(
			    temp16alen, temp16a, temp32alen, temp32a, temp48);
			finlength = fast_expansion_sum_zeroelim(
			    finlength, finnow, temp48len, temp48, finother);
			...
			temp32alen = scale_expansion_zeroelim(bytcatlen, bytcat,
							      bdytail, temp32a);
			bytcattlen = scale_expansion_zeroelim(cattlen, catt,
							      bdytail, bytcatt);
			temp16alen = scale_expansion_zeroelim(
			    bytcattlen, bytcatt, 2.0 * bdy, temp16a);
			temp16blen = scale_expansion_zeroelim(
			    bytcattlen, bytcatt, bdytail, temp16b);
			temp32blen = fast_expansion_sum_zeroelim(
			    temp16alen, temp16a, temp16blen, temp16b, temp32b);
			temp64len = fast_expansion_sum_zeroelim(
			    temp32alen, temp32a, temp32blen, temp32b, temp64);
	...
	if ((cdxtail != 0.0) || (cdytail != 0.0)) {
		if ((adxtail != 0.0) || (adytail != 0.0) || (bdxtail != 0.0) ||
			...
			Two_Two_Diff(ti1, ti0, tj1, tj0, abtt3, abtt[2],
				     abtt[1], abtt[0]);
			abtt[3] = abtt3;
			abttlen = 4;
		} else {
		...
		if (cdxtail != 0.0) {
			temp16alen = scale_expansion_zeroelim(cxtablen, cxtab,
							      cdxtail, temp16a);
			cxtabtlen = scale_expansion_zeroelim(abtlen, abt,
							     cdxtail, cxtabt);
			temp32alen = scale_expansion_zeroelim(
			    cxtabtlen, cxtabt, 2.0 * cdx, temp32a);
			temp48len = fast_expansion_sum_zeroelim(
			    temp16alen, temp16a, temp32alen, temp32a, temp48);
			finlength = fast_expansion_sum_zeroelim(
			    finlength, finnow, temp48len, temp48, finother);
			...
			temp32alen = scale_expansion_zeroelim(cxtabtlen, cxtabt,
							      cdxtail, temp32a);
			cxtabttlen = scale_expansion_zeroelim(abttlen, abtt,
							      cdxtail, cxtabtt);
			temp16alen = scale_expansion_zeroelim(
			    cxtabttlen, cxtabtt, 2.0 * cdx, temp16a);
			temp16blen = scale_expansion_zeroelim(
			    cxtabttlen, cxtabtt, cdxtail, temp16b);
			temp32blen = fast_expansion_sum_zeroelim(
			    temp16alen, temp16a, temp16blen, temp16b, temp32b);
			temp64len = fast_expansion_sum_zeroelim(
			    temp32alen, temp32a, temp32blen, temp32b, temp64);
		...
		if (cdytail != 0.0) {
			temp16alen = scale_expansion_zeroelim(cytablen, cytab,
							      cdytail, temp16a);
			cytabtlen = scale_expansion_zeroelim(abtlen, abt,
							     cdytail, cytabt);
			temp32alen = scale_expansion_zeroelim(
			    cytabtlen, cytabt, 2.0 * cdy, temp32a);
			temp48len = fast_expansion_sum_zeroelim(
			    temp16alen, temp16a, temp32alen, temp32a, temp48);
			finlength = fast_expansion_sum_zeroelim(
			    finlength, finnow, temp48len, temp48, finother);
			...
			temp32alen = scale_expansion_zeroelim(cytabtlen, cytabt,
							      cdytail, temp32a);
			cytabttlen = scale_expansion_zeroelim(abttlen, abtt,
							      cdytail, cytabtt);
			temp16alen = scale_expansion_zeroelim(
			    cytabttlen, cytabtt, 2.0 * cdy, temp16a);
			temp16blen = scale_expansion_zeroelim(
			    cytabttlen, cytabtt, cdytail, temp16b);
			temp32blen = fast_expansion_sum_zeroelim(
			    temp16alen, temp16a, temp16blen, temp16b, temp32b);
			temp64len = fast_expansion_sum_zeroelim(
			    temp32alen, temp32a, temp32blen, temp32b, temp64);
...

{
	...
	if (adxtail != 0.0) {
		if (bdytail != 0.0) {
			...
			Two_One_Product(adxt_bdyt1, adxt_bdyt0, cdheight, u3,
					u[2], u[1], u[0]);
			u[3] = u3;
			finlength = fast_expansion_sum_zeroelim(
		...
		if (cdytail != 0.0) {
			...
			Two_One_Product(adxt_cdyt1, adxt_cdyt0, bdheight, u3,
					u[2], u[1], u[0]);
			u[3] = u3;
			finlength = fast_expansion_sum_zeroelim(
			    finlength, finnow, 4, u, finother);
			...
			finother = finswap;
			if (bdheighttail != 0.0) {
				Two_One_Product(adxt_cdyt1, adxt_cdyt0,
						bdheighttail, u3, u[2], u[1],
						u[0]);
				u[3] = u3;
				finlength = fast_expansion_sum_zeroelim(
				    finlength, finnow, 4, u, finother);
	...
	if (bdxtail != 0.0) {
		if (cdytail != 0.0) {
			...
			Two_One_Product(bdxt_cdyt1, bdxt_cdyt0, adheight, u3,
					u[2], u[1], u[0]);
			u[3] = u3;
			finlength = fast_expansion_sum_zeroelim(
			    finlength, finnow, 4, u, finother);
			...
			finother = finswap;
			if (adheighttail != 0.0) {
				Two_One_Product(bdxt_cdyt1, bdxt_cdyt0,
						adheighttail, u3, u[2], u[1],
						u[0]);
				u[3] = u3;
				finlength = fast_expansion_sum_zeroelim(
				    finlength, finnow, 4, u, finother);
		...
		if (adytail != 0.0) {
			...
			Two_One_Product(bdxt_adyt1, bdxt_adyt0, cdheight, u3,
					u[2], u[1], u[0]);
			u[3] = u3;
			finlength = fast_expansion_sum_zeroelim(
			    finlength, finnow, 4, u, finother);
	...
	if (cdxtail != 0.0) {
		if (adytail != 0.0) {
			...
			Two_One_Product(cdxt_adyt1, cdxt_adyt0, bdheight, u3,
					u[2], u[1], u[0]);
			u[3] = u3;
			finlength = fast_expansion_sum_zeroelim(
			    finlength, finnow, 4, u, finother);
			...
			finother = finswap;
			if (bdheighttail != 0.0) {
				Two_One_Product(cdxt_adyt1, cdxt_adyt0,
						bdheighttail, u3, u[2], u[1],
						u[0]);
				u[3] = u3;
				finlength = fast_expansion_sum_zeroelim(
		...
		if (bdytail != 0.0) {
			...
			Two_One_Product(cdxt_bdyt1, cdxt_bdyt0, adheight, u3,
					u[2], u[1], u[0]);
			u[3] = u3;
			finlength = fast_expansion_sum_zeroelim(
			    finlength, finnow, 4, u, finother);
			...
			finother = finswap;
			if (adheighttail != 0.0) {
				Two_One_Product(cdxt_bdyt1, cdxt_bdyt0,
						adheighttail, u3, u[2], u[1],
						u[0]);
				u[3] = u3;
				finlength = fast_expansion_sum_zeroelim(
...

{
	if (b->weighted == 0) {
		...
		return orient3d(m, b, pa, pb, pc, pd, pa[2], pb[2], pc[2],
				pd[2]);
	}
}
...

{
	...
	if ((dodist < aodist) && (dodist < dadist)) {
		if (offcenter && (b->offconstant > 0.0)) {
			/* Find the position of the off-center, as described by
			 * Alper Ungor. */
		...
		if (offcenter && (b->offconstant > 0.0)) {
			...
			/* If the off-center is closer to the destination than
			 * the */
			/*   circumcenter, use the off-center instead. */
...

{
	...
	m->recenttri.tri =
	    (triangle *)NULL; /* No triangle has been visited yet. */
	m->undeads = 0;	      /* No eliminated input vertices yet. */
	m->samples = 1; /* Point location should take at least one sample. */
	m->checksegments =
	    0; /* There are no segments in the triangulation yet. */
	m->checkquality =
	    0; /* The quality triangulation stage has not begun. */
...

{
	...
	while (triangleloop.tri != (triangle *)NULL) {
		...
		for (triangleloop.orient = 0; triangleloop.orient < 3;
			...
			if (triangleloop.orient ==
			    0) { /* Only test for inversion once. */
				/* Test if the triangle is flat or inverted. */
				apex(triangleloop, triapex);
				if (counterclockwise(m, b, triorg, tridest,
						     triapex) <= 0.0) {
					printf("  !! !! Inverted ");
			...
			if (oppotri.tri != m->dummytri) {
				/* Check that the triangle's neighbor knows it's
				 * a neighbor. */
				...
				/* Check that both triangles agree on the
				 * identities */
				...
				if ((triorg != oppodest) ||
				    (tridest != oppoorg)) {
					printf("  !! !! Mismatched edge "
					       "coordinates between two "
					       "triangles:\n");
	...
	if (horrors == 0) {
		if (!b->quiet) {
			printf("  In my studied opinion, the mesh appears to "
			       "be consistent.\n");
		}
	} else if (horrors == 1) {
		printf("  !! !! !! !! Precisely one festering wound "
		       "discovered.\n");
	} else {
...

{
	...
	while (triangleloop.tri != (triangle *)NULL) {
		...
		for (triangleloop.orient = 0; triangleloop.orient < 3;
			...
			/* Only test that the edge is locally Delaunay if there
			 * is an   */
			/*   adjoining triangle whose pointer is larger (to
			 * ensure that */
			...
			shouldbedelaunay = (oppotri.tri != m->dummytri) &&
					   ...
					   (triorg != m->infvertex2) &&
					   (triorg != m->infvertex3) &&
					   (tridest != m->infvertex1) &&
					   (tridest != m->infvertex2) &&
					   (tridest != m->infvertex3) &&
					   (triapex != m->infvertex1) &&
					   (triapex != m->infvertex2) &&
					   (triapex != m->infvertex3) &&
					   (oppoapex != m->infvertex1) &&
					   (oppoapex != m->infvertex2) &&
					   (oppoapex != m->infvertex3);
			if (m->checksegments && shouldbedelaunay) {
				/* If a subsegment separates the triangles, then
				 * the edge is */
				/*   constrained, so no local Delaunay test
				 * should be done.  */
			...
			if (shouldbedelaunay) {
				if (nonregular(m, b, triorg, tridest, triapex,
					       oppoapex) > 0.0) {
					if (!b->weighted) {
						printf("  !! !! Non-Delaunay "
						       "pair of triangles:\n");
	...
	if (horrors == 0) {
		if (!b->quiet) {
			printf("  By virtue of my perceptive intelligence, I "
			       "declare the mesh Delaunay.\n");
		}
	} else if (horrors == 1) {
		printf("  !! !! !! !! Precisely one terrifying transgression "
		       "identified.\n");
	} else {
...

{
	...
	if (badtri->key >= 1.0) {
		...
		/* `badtri->key' is 2.0 to a negative exponent, so we'll record
		 * that */
		/*   fact and use the reciprocal of `badtri->key', which is
		 * > 1.0.   */
	...
	if (m->queuefront[queuenumber] == (struct badtriang *)NULL) {
		...
		if (queuenumber > m->firstnonemptyq) {
			...
			/* Mark the newly nonempty queue as following a
...

{
	...
	if (neighbortri.tri != m->dummytri) {
		...
		/* Check whether the apex is in the diametral lens of the
		 * subsegment */
		/*   (the diametral circle if `conformdel' is set).  A dot
		 * product   */
		/*   of two sides of the triangle is used to check whether the
		 * angle */
		/*   at the apex is greater than (180 - 2 `minangle') degrees
		 * (for   */
	...
	if (encroached &&
	    (!b->nobisect || ((b->nobisect == 1) && (sides == 2)))) {
		if (b->verbose > 2) {
			printf("  Queueing encroached subsegment (%.12g, "
			       "%.12g) (%.12g, %.12g).\n",
...

{
	...
	REAL apexlen2, orglen2, destlen2, minedge2, maxedge2;
	REAL cos2angle;
	...
	dxao2 = dxao * dxao;
	dyao2 = dyao * dyao;
	/* Find the squared lengths of the triangle's three edges. */
	...
	if ((apexlen2 < orglen2) && (apexlen2 < destlen2)) {
		...
		minedge2 = apexlen2;
		maxedge2 = orglen2 < destlen2 ? destlen2 : orglen2;
		/* Find the square of the cosine of the angle at the apex. */
		cos2angle = dxda * dxao + dyda * dyao;
		cos2angle = cos2angle * cos2angle / (orglen2 * destlen2);
		base1 = torg;
		base2 = tdest;
		otricopy(*testtri, tri1);
	} else if (orglen2 < destlen2) {
		/* The edge opposite the origin is shortest. */
		minedge2 = orglen2;
		maxedge2 = apexlen2 < destlen2 ? destlen2 : apexlen2;
		/* Find the square of the cosine of the angle at the origin. */
		cos2angle = dxod * dxao + dyod * dyao;
		cos2angle = cos2angle * cos2angle / (apexlen2 * destlen2);
		base1 = tdest;
		base2 = tapex;
		lnext(*testtri, tri1);
	} else {
		...
		minedge2 = destlen2;
		maxedge2 = apexlen2 < orglen2 ? orglen2 : apexlen2;
		/* Find the square of the cosine of the angle at the
		 * destination. */
		cos2angle = dxod * dxda + dyod * dyda;
		cos2angle = cos2angle * cos2angle / (apexlen2 * orglen2);
		base1 = tapex;
		base2 = torg;
		lprev(*testtri, tri1);
	}
	...
	minx = (tdest[0] < minx) ? tdest[0] : minx;
	maxy = (torg[1] > tapex[1]) ? torg[1] : tapex[1];
	maxy = (tdest[1] > maxy) ? tdest[1] : maxy;
...

{
	...
	while (1) {
		...
		if ((fapex[0] == searchpoint[0]) &&
		    (fapex[1] == searchpoint[1])) {
			lprevself(*searchtri);
			return ONVERTEX;
		}
...

{
	...
	if (m->recenttri.tri != (triangle *)NULL) {
		if (!deadtri(m->recenttri.tri)) {
			...
			dist = (searchpoint[0] - torg[0]) *
				   (searchpoint[0] - torg[0]) +
			       (searchpoint[1] - torg[1]) *
				   (searchpoint[1] - torg[1]);
			if (dist < searchdist) {
				otricopy(m->recenttri, *searchtri);
				searchdist = dist;
	...
	while (totalsamplesleft > 0) {
		/* If we're in the last block, `population' needs to be
		 * corrected. */
...

{
	...
	if (top.tri == m->dummytri) {
		printf("  Internal error in flip():  Attempt to flip on "
		       "boundary.\n");
		lnextself(*flipedge);
		return;
	}
...
#ifdef SELF_CHECK
	if (top.tri == m->dummytri) {
		printf("  Internal error in unflip():  Attempt to flip on "
		       "boundary.\n");
		lnextself(*flipedge);
		return;
	}
...

{
	...
	if ((intersect == ONEDGE) || (intersect == OUTSIDE)) {
				...
				if (segmentflaws) {
					...
					if (enq) {
						...
						encroached =
							...
							poolalloc(
							    &m->badsubsegs);
						encroached->encsubseg =
						    sencode(brokensubseg);
						sorg(brokensubseg,
						     encroached->subsegorg);
						sdest(brokensubseg,
...
#ifdef SELF_CHECK
		if (counterclockwise(m, b, rightvertex, leftvertex, botvertex) <
		    0.0) {
			printf("  Internal error in insertvertex():\n");
			printf("  Clockwise triangle prior to edge vertex "
			       "insertion (bottom).\n");
		}
		if (mirrorflag) {
		...
		if (counterclockwise(m, b, rightvertex, leftvertex, botvertex) <
		    0.0) {
			printf("  Internal error in insertvertex():\n");
			printf("  Clockwise triangle prior to vertex "
			       "insertion.\n");
		}
	...
	while (1) {
		...
		if (!doflip) {
			...
			/* Finish finding the next edge around the newly
			 * inserted vertex. */
			lnext(testtri, horiz);
...

{
	...
	if (b->verbose > 2) {
		...
		printf("    (%.12g, %.12g) (%.12g, %.12g)\n", leftbasevertex[0],
		       leftbasevertex[1], rightbasevertex[0],
		       rightbasevertex[1]);
	}
...

{
	...
	if (arraysize - divider >= 2) {
		...
		alternateaxes(&sortarray[divider], arraysize - divider,
			      1 - axis);
	}
}
...

{
	...
	/* Connect it to the bounding boxes of the left and right
	 * triangulations. */
	...
	while (1) {
		...
		leftfinished = counterclockwise(m, b, upperleft, lowerleft,
						lowerright) <= 0.0;
		rightfinished = counterclockwise(m, b, upperright, lowerleft,
						 lowerright) <= 0.0;
		if (leftfinished && rightfinished) {
			...
			/* Apex is intentionally left NULL. */
			/* Connect it to the bounding boxes of the two
			 * triangulations. */
		...
		if (leftfinished ||
		    (!rightfinished &&
		     (incircle(m, b, upperleft, lowerleft, lowerright,
			       upperright) > 0.0))) {
			/* Knit the triangulations, adding an edge from
			 * `lowerleft' */
			/*   to `upperright'. */
...

{
	...
	if (vertices == 2) {
		...
		if (area == 0.0) {
			...
			/* The three vertices are not collinear; the
		...
		divconqrecurse(m, b, sortarray, divider, 1 - axis, farleft,
			       &innerleft);
		...
		mergehulls(m, b, farleft, &innerleft, &innerright, farright,
			   axis);
	}
}
...

{
	...
	while (vertexloop != (vertex)NULL) {
		...
		if (insertvertex(m, b, vertexloop, &starttri,
				 (struct osub *)NULL, 0,
				 0) == DUPLICATEVERTEX) {
			if (b->verbose > 1) {
				printf("WARNING:  A duplicate vertex "
...

{
	...
	if (checkvertex == splaytree->keydest) {
		rightofroot =
		    rightofhyperbola(m, &splaytree->keyedge, searchpoint);
		if (rightofroot) {
		...
		rightofchild =
		    rightofhyperbola(m, &child->keyedge, searchpoint);
		if (rightofchild) {
			otricopy(child->keyedge, *searchtri);
			grandchild =
			    splay(m, child->rchild, searchpoint, searchtri);
			child->rchild = grandchild;
		} else {
			grandchild =
			    splay(m, child->lchild, searchpoint, searchtri);
...

{
	...
	searchpoint[1] = topy;
	return splayinsert(m,
			   splay(m, splayroot, (vertex)searchpoint, &dummytri),
...

{
	...
	while (heapsize > 0) {
		...
		if (nextevent->xkey < m->xmin) {
			...
			check4deadevent(&farlefttri, &freeevents, eventheap,
					&heapsize);
			onext(fliptri, farrighttri);
			check4deadevent(&farrighttri, &freeevents, eventheap,
					&heapsize);
			...
			if (randomnation(SAMPLERATE) == 0) {
				...
				splayroot = circletopinsert(
				    m, b, splayroot, &lefttri, leftvertex,
			...
			if ((nextvertex[0] == lastvertex[0]) &&
				...
				*/
				...
				check4deadevent(&searchtri, &freeevents,
						eventheap, &heapsize);
				...
				if (randomnation(SAMPLERATE) == 0) {
					splayroot = splayinsert(
					    m, splayroot, &lefttri, nextvertex);
				} else if (randomnation(SAMPLERATE) == 0) {
					lnext(righttri, inserttri);
					splayroot =
					    splayinsert(m, splayroot,
							&inserttri, nextvertex);
				}
			}
		}
		...
		if (check4events) {
			...
			lefttest = counterclockwise(m, b, leftvertex, midvertex,
						    rightvertex);
			if (lefttest > 0.0) {
				newevent = freeevents;
				freeevents =
				...
				newevent->ykey =
				    circletop(m, leftvertex, midvertex,
					      rightvertex, lefttest);
			...
			righttest = counterclockwise(m, b, leftvertex,
						     midvertex, rightvertex);
			if (righttest > 0.0) {
				newevent = freeevents;
				freeevents =
				    (struct event *)freeevents->eventptr;
				...
				newevent->ykey =
				    circletop(m, leftvertex, midvertex,
					      rightvertex, righttest);
				newevent->eventptr =
				    (VOID *)encode(farrighttri);
				eventheapinsert(eventheap, heapsize, newevent);
				heapsize++;
...

{
	...
	/*if (!b->quiet) {*/
	if (1 == 1) {
		printf("   Constructing Delaunay triangulation by "
		       "divide-and-conquer method.\n");
	}
...

{
	...
	for (elementnumber = 1; elementnumber <= m->inelements;
	     elementnumber++) {
	...
	if (b->poly) {
		...
		for (segmentnumber = 1; segmentnumber <= m->insegments;
		     segmentnumber++) {
	...
	if (b->vararea) {
		...
		if (areaelements != m->inelements) {
			printf("ERROR:  %s and %s disagree on number of "
			       "triangles.\n",
	...
	for (j = 0; j < m->eextras; j++) {
		...
		setelemattribute(triangleloop, j,
				 triangleattriblist[attribindex++]);
	#else  /* not TRILIBRARY */
				stringptr = findfield(stringptr);
				if (*stringptr == '\0') {
			...
			boundmarker =
				...
				if (segmentmarkers) {
					...
					if (*stringptr == '\0') {
						...
						boundmarker = (int)strtol(
						    stringptr, &stringptr, 0);
					}
				}
	#endif /* not TRILIBRARY */
		for (j = 0; j < 2; j++) {
...

{
	...
	if (leftflag && rightflag) {
		/* `searchtri' faces directly away from `searchpoint'.  We could
		 * go left */
		/*   or right.  Ask whether it's a triangle or a boundary on the
		 * left.   */
	...
	while (leftflag) {
		...
		onextself(*searchtri);
		if (searchtri->tri == m->dummytri) {
			printf("   Internal error in finddirection():  Unable "
			       "to find a\n");
		...
		apex(*searchtri, leftvertex);
		rightccw = leftccw;
		leftccw = counterclockwise(m, b, searchpoint, startvertex,
					   leftvertex);
		leftflag = leftccw > 0.0;
	}
	while (rightflag) {
		...
		dest(*searchtri, rightvertex);
		leftccw = rightccw;
		rightccw = counterclockwise(m, b, startvertex, searchpoint,
					    rightvertex);
		rightflag = rightccw > 0.0;
	}
	if (leftccw == 0.0) {
...

{
	...
	if ((leftvertex[0] == endpoint1[0]) &&
	    (leftvertex[1] == endpoint1[1])) {
		onextself(*splittri);
...

{
	...
	dest(*searchtri, rightvertex);
	apex(*searchtri, leftvertex);
	if (((leftvertex[0] == endpoint2[0]) &&
	     (leftvertex[1] == endpoint2[1])) ||
	    ((rightvertex[0] == endpoint2[0]) &&
	     (rightvertex[1] == endpoint2[1]))) {
		...
		if (crosssubseg.ss == m->dummysub) {
			...
			segmentintersection(m, b, &crosstri, &crosssubseg,
					    endpoint2);
			otricopy(crosstri, *searchtri);
			insertsubseg(m, b, searchtri, newmark);
			/* Insert the remainder of the segment. */
			return scoutsegment(m, b, searchtri, endpoint2,
					    newmark);
		}
	}
}
...

{
	...
	if (b->verbose > 2) {
		printf("Forcing segment into triangulation by recursive "
		       "splitting:\n");
		printf("  (%.12g, %.12g) (%.12g, %.12g)\n", endpoint1[0],
	...
	success = insertvertex(m, b, newvertex, &searchtri1,
			       (struct osub *)NULL, 0, 0);
	if (success == DUPLICATEVERTEX) {
		if (b->verbose > 2) {
			printf("  Segment intersects existing vertex (%.12g, "
			       "%.12g).\n",
		...
		if (success == VIOLATINGVERTEX) {
			if (b->verbose > 2) {
				printf("  Two segments intersect at (%.12g, "
				       "%.12g).\n",
				       newvertex[0], newvertex[1]);
			}
			/* By fluke, we've landed right on another segment.
			 * Split it. */
	...
	/*   `newvertex', and will be directed toward `endpoint1' and
	 * `endpoint2' */
	...
	if (!scoutsegment(m, b, &searchtri1, endpoint1, newmark)) {
		/* The origin of searchtri1 may have changed if a collision with
		 * an */
	...
	if (!scoutsegment(m, b, &searchtri2, endpoint2, newmark)) {
		/* The origin of searchtri2 may have changed if a collision with
		 * an */
		/*   intervening vertex on the segment occurred. */
...

{
	...
	if (leftside) {
		if (counterclockwise(m, b, nearvertex, leftvertex, farvertex) <=
		    0.0) {
		...
		if (counterclockwise(m, b, farvertex, rightvertex,
				     nearvertex) <= 0.0) {
	...
	if (counterclockwise(m, b, rightvertex, leftvertex, farvertex) > 0.0) {
		/* fartri is not an inverted triangle, and farvertex is not a
		 * reflex */
		/*   vertex.  As there are no reflex vertices, fixuptri isn't an
		 */
		/*   inverted triangle, either.  Hence, test the edge between
		 * the    */
		/*   triangles to ensure it is locally Delaunay. */
		if (incircle(m, b, leftvertex, farvertex, rightvertex,
			     nearvertex) <= 0.0) {
			return;
		}
	...
	flip(m, b, &neartri);
	lprevself(
	    *fixuptri); /* Restore the origin of fixuptri after the flip. */
...

{
	...
	do {
		...
		/* `farvertex' is the extreme point of the polygon we are
		 * "digging" */
		/*   to get from endpoint1 to endpoint2. */
		if ((farvertex[0] == endpoint2[0]) &&
		    (farvertex[1] == endpoint2[1])) {
			...
			/* Check whether farvertex is to the left or right of
			 * the segment */
			/*   being inserted, to decide which edge of fixuptri to
			 * dig      */
			/*   through next. */
			area = counterclockwise(m, b, endpoint1, endpoint2,
						farvertex);
			if (area == 0.0) {
				/* We've collided with a vertex between
				 * endpoint1 and endpoint2. */
				collision = 1;
				oprev(fixuptri, fixuptri2);
				/* Enforce the Delaunay condition around
				 * farvertex. */
				delaunayfixup(m, b, &fixuptri, 0);
				delaunayfixup(m, b, &fixuptri2, 1);
				done = 1;
			} else {
				if (area > 0.0) { /* farvertex is to the left of
						     the segment. */
					oprev(fixuptri, fixuptri2);
					/* Enforce the Delaunay condition around
					 * farvertex, on the */
					...
					delaunayfixup(m, b, &fixuptri2, 1);
					/* Flip the edge that crosses the
					 * segment.  After the edge is */
					/*   flipped, one of its endpoints is
					 * the fan vertex, and the */
					/*   destination of fixuptri is the fan
					 * vertex.               */
					lprevself(fixuptri);
				} else { /* farvertex is to the right of the
					    segment. */
					delaunayfixup(m, b, &fixuptri, 0);
					/* Flip the edge that crosses the
					 * segment.  After the edge is */
				...
				if (crosssubseg.ss == m->dummysub) {
					flip(m, b,
					     &fixuptri); /* May create inverted
							    triangle at left. */
				} else {
					/* We've collided with a segment between
					 * endpoint1 and endpoint2. */
					collision = 1;
					/* Insert a vertex at the intersection.
					 */
					segmentintersection(m, b, &fixuptri,
							    &crosssubseg,
							    endpoint2);
					done = 1;
				}
...

{
	...
	if (checkvertex != endpoint1) {
		...
		if (locate(m, b, endpoint1, &searchtri1) != ONVERTEX) {
			printf("  Internal error in insertsegment():  Unable "
	...
	/* The first endpoint may have changed if a collision with an
	 * intervening */
...

{
	...
	do {
		...
		/* To find the next hull edge, go clockwise around the next
		 * vertex. */
		lnextself(hulltri);
...

{
	...
	if (b->poly) {
		/*if (!b->quiet) {*/
		if (1 == 1) {
			printf("   Recovering segments in Delaunay "
			       "triangulation.\n");
			fflush(stdout);
		}
#ifdef TRILIBRARY
...
#endif /* not TRILIBRARY */
		/* If the input vertices are collinear, there is no
		 * triangulation, */
		...
		for (i = 0; i < m->insegments; i++) {
...
#else  /* not TRILIBRARY */
			stringptr =
			    readline(inputline, polyfile, b->inpolyfilename);
			stringptr = findfield(stringptr);
			if (*stringptr == '\0') {
				printf("Error:  Segment %d has no endpoints in "
				       "%s.\n",
			...
			stringptr = findfield(stringptr);
			if (*stringptr == '\0') {
				printf("Error:  Segment %d is missing its "
				       "second endpoint in %s.\n",
				       b->firstnumber + i, polyfilename);
				triexit(1);
			} else {
			...
			if ((end1 < b->firstnumber) ||
				...
				if (b->verbose > 1) {
					printf(
					    "WARNING:  Invalid first endpoint "
					    "of segment %d in %s.\n",
					    b->firstnumber + i, polyfilename);
				}
				...
				if (b->verbose > 1) {
					printf(
					    "WARNING:  Invalid second endpoint "
					    "of segment %d in %s.\n",
					    b->firstnumber + i, polyfilename);
				}
			} else {
				...
				if ((endpoint1[0] == endpoint2[0]) &&
					...
					insertsegment(m, b, endpoint1,
						      endpoint2, boundmarker);
				}
			}
		}
	} else {
...

{
	...
	if (b->verbose) {
		printf("   Marking concavities (external triangles) for "
		       "elimination.\n");
	}
	/* Find a triangle handle on the hull. */
	hulltri.tri = m->dummytri;
		...
		if (!infected(hulltri)) {
			...
			if (hullsubseg.ss == m->dummysub) {
				...
				if (!infected(hulltri)) {
					infect(hulltri);
					deadtriangle =
					    (triangle **)poolalloc(&m->viri);
					*deadtriangle = hulltri.tri;
				}
			} else {
				/* The triangle is protected; set boundary
...

{
	...
	while (virusloop != (triangle **)NULL) {
		...
		/* A triangle is marked as infected by messing with one of its
		 * pointers */
		/*   to subsegments, setting it to an illegal value.  Hence, we
		 * have to */
		/*   temporarily uninfect this triangle so that we can examine
		 * its      */
		...
		if (b->verbose > 2) {
			...
			printf("    Checking (%.12g, %.12g) (%.12g, %.12g) "
			       "(%.12g, %.12g)\n",
		...
		for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
			...
			/* Check for a subsegment between the triangle and its
			 * neighbor. */
			tspivot(testtri, neighborsubseg);
			/* Check if the neighbor is nonexistent or already
			 * infected. */
			if ((neighbor.tri == m->dummytri) ||
			    infected(neighbor)) {
				if (neighborsubseg.ss != m->dummysub) {
					/* There is a subsegment separating the
					 * triangle from its      */
					/*   neighbor, but both triangles are
					 * dying, so the subsegment */
					/*   dies too. */
					subsegdealloc(m, neighborsubseg.ss);
					if (neighbor.tri != m->dummytri) {
						/* Make sure the subsegment
						 * doesn't get deallocated again
						 */
						/*   later when the infected
						 * neighbor is visited. */
						uninfect(neighbor);
						tsdissolve(neighbor);
						infect(neighbor);
				...
				if (neighborsubseg.ss == m->dummysub) {
					/* There is no subsegment protecting the
					 * neighbor, so */
					...
					if (b->verbose > 2) {
						...
						printf("    Marking (%.12g, "
						       ...
						       "(%.12g, %.12g)\n",
						       deadorg[0], deadorg[1],
						       deaddest[0], deaddest[1],
						       deadapex[0],
						       deadapex[1]);
					}
					infect(neighbor);
					/* Ensure that the neighbor's neighbors
					 * will be infected. */
					deadtriangle =
					    (triangle **)poolalloc(&m->viri);
					*deadtriangle = neighbor.tri;
				} else { /* The neighbor is protected by a
					    subsegment. */
					/* Remove this triangle from the
					 * subsegment. */
					stdissolve(neighborsubseg);
					/* The subsegment becomes a boundary.
					 * Set markers accordingly. */
		...
		/* Remark the triangle as infected, so it doesn't get added to
		 * the */
		/*   virus pool again. */
		infect(testtri);
		virusloop = (triangle **)traverse(&m->viri);
	...
	while (virusloop != (triangle **)NULL) {
		...
		/* Check each of the three corners of the triangle for
		 * elimination. */
		/*   This is done by walking around each vertex, checking if it
		 * is  */
		...
		for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
			...
			if (testvertex != (vertex)NULL) {
				killorg = 1;
				/* Mark the corner of the triangle as having
				 * been tested. */
				setorg(testtri, NULL);
				/* Walk counterclockwise about the vertex. */
				onext(testtri, neighbor);
				/* Stop upon reaching a boundary or the starting
				 * triangle. */
				while ((neighbor.tri != m->dummytri) &&
				       (!otriequal(neighbor, testtri))) {
					if (infected(neighbor)) {
						/* Mark the corner of this
						 * triangle as having been
						 * tested. */
						setorg(neighbor, NULL);
					} else {
						/* A live triangle.  The vertex
						 * survives. */
						killorg = 0;
					}
					/* Walk counterclockwise about the
					 * vertex. */
					onextself(neighbor);
				}
				/* If we reached a boundary, we must walk
				 * clockwise as well. */
				if (neighbor.tri == m->dummytri) {
		...
		/* Record changes in the number of boundary edges, and
		 * disconnect */
		/*   dead triangles from their neighbors. */
		for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
			sym(testtri, neighbor);
			if (neighbor.tri == m->dummytri) {
				/* There is no neighboring triangle on this
				 * edge, so this edge    */
				/*   is a boundary edge.  This triangle is being
				 * deleted, so this */
				...
				dissolve(neighbor);
				/* There is a neighboring triangle on this edge,
				 * so this edge */
				/*   becomes a boundary edge when this triangle
				 * is deleted.   */
				m->hullsize++;
			}
		}
...

{
	...
	while (virusloop != (triangle **)NULL) {
		...
		if (b->verbose > 2) {
			...
			printf("    Checking (%.12g, %.12g) (%.12g, %.12g) "
			       "(%.12g, %.12g)\n",
			       regionorg[0], regionorg[1], regiondest[0],
			       regiondest[1], regionapex[0], regionapex[1]);
		}
		/* Check each of the triangle's three neighbors. */
		for (testtri.orient = 0; testtri.orient < 3; testtri.orient++) {
			/* Find the neighbor. */
			 ...
			 * neighbor. */
			tspivot(testtri, neighborsubseg);
			/* Make sure the neighbor exists, is not already
			 * infected, and */
			...
			if ((neighbor.tri != m->dummytri) &&
				...
				if (b->verbose > 2) {
					...
					printf(
					    "    Marking (%.12g, %.12g) "
					    "(%.12g, %.12g) (%.12g, %.12g)\n",
					    regionorg[0], regionorg[1],
					    regiondest[0], regiondest[1],
					    regionapex[0], regionapex[1]);
				}
				/* Infect the neighbor. */
				infect(neighbor);
				/* Ensure that the neighbor's neighbors will be
				 * infected. */
...

{
	...
	if (regions > 0) {
		/* Allocate storage for the triangles in which region points
		 * fall. */
	...
	if (((holes > 0) && !b->noholes) || !b->convex || (regions > 0)) {
		...
		poolinit(&m->viri, sizeof(triangle *), VIRUSPERBLOCK,
			 VIRUSPERBLOCK, 0);
	}
	...
	if ((holes > 0) && !b->noholes) {
		...
		for (i = 0; i < 2 * holes; i += 2) {
			/* Ignore holes that aren't within the bounds of the
			 * mesh. */
			if ((holelist[i] >= m->xmin) &&
			    (holelist[i] <= m->xmax) &&
			    (holelist[i + 1] >= m->ymin) &&
			    (holelist[i + 1] <= m->ymax)) {
				/* Start searching from some triangle on the
				 * outer boundary. */
				searchtri.tri = m->dummytri;
				searchtri.orient = 0;
				symself(searchtri);
				/* Ensure that the hole is to the left of this
				 * boundary edge; */
				/*   otherwise, locate() will falsely report
				 * that the hole    */
				/*   falls within the starting triangle. */
				org(searchtri, searchorg);
				dest(searchtri, searchdest);
				if (counterclockwise(m, b, searchorg,
						     searchdest,
						     &holelist[i]) > 0.0) {
					/* Find a triangle that contains the
					 * hole. */
					intersect = locate(m, b, &holelist[i],
							   &searchtri);
					if ((intersect != OUTSIDE) &&
					    (!infected(searchtri))) {
						/* Infect the triangle.  This is
						 * done by marking the triangle
						 */
						/*   as infected and including
						 * the triangle in the virus
						 * pool. */
						infect(searchtri);
						holetri =
						    (triangle **)poolalloc(
	...
	if (regions > 0) {
		...
		for (i = 0; i < regions; i++) {
			...
			/* Ignore region points that aren't within the bounds of
			 * the mesh. */
			if ((regionlist[4 * i] >= m->xmin) &&
				...
				searchtri.orient = 0;
				symself(searchtri);
				/* Ensure that the region point is to the left
				 * of this boundary */
				/*   edge; otherwise, locate() will falsely
				 * report that the     */
				/*   region point falls within the starting
				 * triangle.           */
				org(searchtri, searchorg);
				dest(searchtri, searchdest);
				if (counterclockwise(
					m, b, searchorg, searchdest,
					&regionlist[4 * i]) > 0.0) {
					/* Find a triangle that contains the
					 * region point. */
					intersect =
					    locate(m, b, &regionlist[4 * i],
						   &searchtri);
					if ((intersect != OUTSIDE) &&
					    (!infected(searchtri))) {
						/* Record the triangle for
	...
	if (regions > 0) {
		...
		if (1 == 1) {
			if (b->regionattrib) {
				if (b->vararea) {
					...
					printf("   Spreading regional "
					       "attributes.\n");
					fflush(stdout);
				}
			} else {
				printf("   Spreading regional area "
				       "constraints.\n");
			}
		}
		if (b->regionattrib && !b->refine) {
			...
			triangleloop.tri = triangletraverse(m);
			while (triangleloop.tri != (triangle *)NULL) {
				setelemattributeinit(triangleloop, m->eextras,
						     0.0);
				triangleloop.tri = triangletraverse(m);
			}
		}
			...
			if (regiontris[i].tri != m->dummytri) {
				...
				if (!deadtri(regiontris[i].tri)) {
					...
					*regiontri = regiontris[i].tri;
					/* Apply one region's attribute and/or
					 * area constraint. */
					regionplague(m, b,
						     regionlist[4 * i + 2],
						     regionlist[4 * i + 3]);
		...
		if (b->regionattrib && !b->refine) {
			/* Note the fact that each triangle has an additional
			 * attribute. */
			m->eextras++;
		}
	}
...

{
	...
	while ((m->badsubsegs.items > 0) && (m->steinerleft != 0)) {
		...
		while ((encloop != (struct badsubseg *)NULL) &&
		       (m->steinerleft != 0)) {
			sdecode(encloop->encsubseg, currentenc);
			sorg(currentenc, eorg);
			sdest(currentenc, edest);
			/* Make sure that this segment is still the same segment
			 * it was   */
			/*   when it was determined to be encroached.  If the
			 * segment was */
			/*   enqueued multiple times (because several newly
			 * inserted      */
			/*   vertices encroached it), it may have already been
			 * split.     */
			if (!deadsubseg(currentenc.ss) &&
			    (eorg == encloop->subsegorg) &&
			    (edest == encloop->subsegdest)) {
				/* To decide where to split a segment, we need
				 * to know if the   */
				/*   segment shares an endpoint with an adjacent
				 * segment.       */
				/*   The concern is that, if we simply split
				 * every encroached   */
				/*   segment in its center, two adjacent
				 * segments with a small  */
				/*   angle between them might lead to an
				 * infinite loop; each    */
				/*   vertex added to split one segment will
				 * encroach upon the   */
				/*   other segment, which must then be split
				 * with a vertex that */
				/*   will encroach upon the first segment, and
				 * so on forever.   */
				/* To avoid this, imagine a set of concentric
				 * circles, whose    */
				/*   radii are powers of two, about each segment
				 * endpoint.      */
				/*   These concentric circles determine where
				 * the segment is    */
				/*   split.  (If both endpoints are shared with
				 * adjacent        */
				/*   segments, split the segment in the middle,
				 * and apply the   */
				...
				/* Is the destination shared with another
				 * segment? */
				lnextself(testtri);
				tspivot(testtri, testsh);
				acutedest = testsh.ss != m->dummysub;

				/* If we're using Chew's algorithm (rather than
				 * Ruppert's) */
				/*   to define encroachment, delete free
				 * vertices from the */
				...
				/* Now, check the other side of the segment, if
				 * there's a triangle */
				/*   there. */
				sym(enctri, testtri);
				if (testtri.tri != m->dummytri) {
					/* Is the destination shared with
					 * another segment? */
					lnextself(testtri);
					tspivot(testtri, testsh);
					acutedest2 = testsh.ss != m->dummysub;
					acutedest = acutedest || acutedest2;
					/* Is the origin shared with another
					 * segment? */
					lnextself(testtri);
					tspivot(testtri, testsh);
					...
					/* Delete free vertices from the
					 * subsegment's diametral circle. */
					if (!b->conformdel && !acuteorg2 &&
					    !acutedest2) {
						org(testtri, eapex);
						while (
						    (vertextype(eapex) ==
				...
				/* Use the concentric circles if exactly one
				 * endpoint is shared */
				...
				if (acuteorg || acutedest) {
					segmentlength =
					    sqrt((edest[0] - eorg[0]) *
						 ...
						 (edest[1] - eorg[1]) *
						     (edest[1] - eorg[1]));
					/* Find the power of two that most
					 * evenly splits the segment.  */
					/*   The worst case is a 2:1 ratio
					 * between subsegment lengths. */
					nearestpoweroftwo = 1.0;
					while (segmentlength >
					       3.0 * nearestpoweroftwo) {
						nearestpoweroftwo *= 2.0;
					}
					while (segmentlength <
					       1.5 * nearestpoweroftwo) {
						nearestpoweroftwo *= 0.5;
					}
					/* Where do we split the segment? */
					split =
					    nearestpoweroftwo / segmentlength;
					if (acutedest) {
						split = 1.0 - split;
					}
				} else {
					/* If we're not worried about adjacent
					 * segments, split */
				...
				for (i = 0; i < 2 + m->nextras; i++) {
					newvertex[i] =
					    eorg[i] +
					    split * (edest[i] - eorg[i]);
				}

				if (!b->noexact) {
					/* Roundoff in the above calculation may
					 * yield a `newvertex'   */
					/*   that is not precisely collinear
					 * with `eorg' and `edest'.  */
					/*   Improve collinearity by one step of
					 * iterative refinement. */
					multiplier = counterclockwise(
					    m, b, eorg, edest, newvertex);
					divisor = ((eorg[0] - edest[0]) *
						       (eorg[0] - edest[0]) +
						   (eorg[1] - edest[1]) *
						       (eorg[1] - edest[1]));
					if ((multiplier != 0.0) &&
				...
				setvertextype(newvertex, SEGMENTVERTEX);
				if (b->verbose > 1) {
					printf("   Splitting subsegment "
					       "(%.12g, %.12g) (%.12g, %.12g) "
					       "at (%.12g, %.12g).\n",
					       eorg[0], eorg[1], edest[0],
					       edest[1], newvertex[0],
					       newvertex[1]);
				}
				/* Check whether the new vertex lies on an
				 * endpoint. */
				if (((newvertex[0] == eorg[0]) &&
				     (newvertex[1] == eorg[1])) ||
				    ((newvertex[0] == edest[0]) &&
				     (newvertex[1] == edest[1]))) {
					printf("ERROR:  Ran out of precision "
					       "at (%.12g, %.12g).\n",
					       newvertex[0], newvertex[1]);
					printf("I attempted to split a segment "
					       "to a smaller size than\n");
					printf("  can be accommodated by the "
					       "finite precision of\n");
					printf(
				...
				/* Insert the splitting vertex.  This should
				 * always succeed. */
				success =
				    insertvertex(m, b, newvertex, &enctri,
						 &currentenc, 1, triflaws);
				if ((success != SUCCESSFULVERTEX) &&
				    (success != ENCROACHINGVERTEX)) {
					printf("   Internal error in "
					       "splitencsegs():\n");
				...
				/* Check the two new subsegments to see if
				 * they're encroached. */
...

{
	...
	if (!deadtri(badotri.tri) && (borg == badtri->triangorg) &&
		...
		if (b->verbose > 1) {
			printf("   Splitting this triangle at its "
			       "circumcenter:\n");
			printf("    (%.12g, %.12g) (%.12g, %.12g) (%.12g, "
			       "%.12g)\n",
			       borg[0], borg[1], bdest[0], bdest[1], bapex[0],
			       bapex[1]);
		}
		...
		newvertex = (vertex)poolalloc(&m->vertices);
		findcircumcenter(m, b, borg, bdest, bapex, newvertex, &xi, &eta,
				 1);
		...
		if (((newvertex[0] == borg[0]) && (newvertex[1] == borg[1])) ||
		    ...
		    ((newvertex[0] == bapex[0]) &&
		     (newvertex[1] == bapex[1]))) {
			if (b->verbose > 1) {
				printf("WARNING:  New vertex (%.12g, %.12g) "
				       "falls on existing vertex.\n",
				       newvertex[0], newvertex[1]);
				errorflag = 1;
			}
			vertexdealloc(m, newvertex);
			...
			for (i = 2; i < 2 + m->nextras; i++) {
				/* Interpolate the vertex attributes at the
				 * circumcenter. */
				newvertex[i] = borg[i] +
					       xi * (bdest[i] - borg[i]) +
					       eta * (bapex[i] - borg[i]);
			}
			/* The new vertex must be in the interior, and therefore
			 * is a */
			/*   free vertex with a marker of zero. */
			setvertexmark(newvertex, 0);
			setvertextype(newvertex, FREEVERTEX);

			/* Ensure that the handle `badotri' does not represent
			 * the longest  */
			/*   edge of the triangle.  This ensures that the
			 * circumcenter must */
			/*   fall to the left of this edge, so point location
			 * will work.    */
			/*   (If the angle org-apex-dest exceeds 90 degrees,
			 * then the       */
			/*   circumcenter lies outside the org-dest edge, and
			 * eta is        */
			/*   negative.  Roundoff error might prevent eta from
			 * being         */
			/*   negative when it should be, so I test eta against
			 * xi.)         */
			if (eta < xi) {
				lprevself(badotri);
			}

			/* Insert the circumcenter, searching from the edge of
			 * the triangle, */
			/*   and maintain the Delaunay property of the
			 * triangulation.        */
			...
			if (success == SUCCESSFULVERTEX) {
				...
				/* If the newly inserted vertex encroaches upon
				 * a subsegment, */
				...
				/* Failed to insert the new vertex, but some
				 * subsegment was */
				/*   marked as being encroached. */
				vertexdealloc(m, newvertex);
			} else { /* success == DUPLICATEVERTEX */
				/* Couldn't insert the new vertex because a
				 * vertex is already there. */
				if (!b->quiet) {
					printf("WARNING:  New vertex (%.12g, "
					       "%.12g) falls on existing "
					       "vertex.\n",
					       newvertex[0], newvertex[1]);
					errorflag = 1;
				}
		...
		if (errorflag) {
			if (b->verbose) {
				printf("  The new vertex is at the "
				       "circumcenter of triangle\n");
				printf("    (%.12g, %.12g) (%.12g, %.12g) "
				       "(%.12g, %.12g)\n",
				       borg[0], borg[1], bdest[0], bdest[1],
				       bapex[0], bapex[1]);
			}
			printf("This probably means that I am trying to refine "
			       "triangles\n");
			printf("  to a smaller size than can be accommodated "
			       "by the finite\n");
...

{
	...
	if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest) {
		...
		poolinit(&m->badtriangles, sizeof(struct badtriang),
			 BADTRIPERBLOCK, BADTRIPERBLOCK, 0);
		...
		while ((m->badtriangles.items > 0) && (m->steinerleft != 0)) {
			/* Fix one bad triangle by inserting a vertex at its
			 * circumcenter. */
			badtri = dequeuebadtriang(m);
			splittriangle(m, b, badtri);
			if (m->badsubsegs.items > 0) {
				/* Put bad triangle back in queue for another
				 * try later. */
	 ...
	 */
	/*   of Steiner points, the triangulation should be (conforming)
	 * Delaunay */
	...
	if (!b->quiet && b->conformdel && (m->badsubsegs.items > 0) &&
	    (m->steinerleft == 0)) {
		printf("\nWARNING:  I ran out of Steiner points, but the mesh "
		       "has\n");
		if (m->badsubsegs.items == 1) {
			printf("  one encroached subsegment, and therefore "
			       "might not be truly\n");
		} else {
			printf("  %ld encroached subsegments, and therefore "
			       "might not be truly\n",
			       m->badsubsegs.items);
		}
		printf("  Delaunay.  If the Delaunay property is important to "
		       "you,\n");
		printf("  try increasing the number of Steiner points "
		       "(controlled by\n");
...

{
	...
	while (triangleloop.tri != (triangle *)NULL) {
		for (triangleloop.orient = 0; triangleloop.orient < 3;
			...
			if ((triangleloop.tri < trisym.tri) ||
				...
				/* Set the new node's marker to zero or one,
				 * depending on */
				/*   whether it lies on a boundary. */
				setvertexmark(newvertex,
					      trisym.tri == m->dummytri);
				...
				if (b->usesegments) {
					...
					/* If this edge is a segment, transfer
					 * the marker to the new node. */
				...
				/* Record the new node in the (one or two)
				 * adjacent elements. */
				triangleloop.tri[m->highorderindex +
						 triangleloop.orient] =
				    (triangle)newvertex;
				if (trisym.tri != m->dummytri) {
					trisym.tri[m->highorderindex +
						   trisym.orient] =
					    (triangle)newvertex;
				}
			}
...

{
	...
	do {
		...
		/* If it's a comment or end of line, read another line and try
		 * again. */
...

{
	...
	if (b->poly) {
		...
		*polyfile = fopen(polyfilename, "r");
		...
		/* Read number of vertices, number of dimensions, number of
		 * vertex */
		...
		if (m->invertices > 0) {
			...
			/* If the .poly file claims there are zero vertices,
			 * that means that */
			/*   the vertices should be read from a separate .node
			 * file.         */
			m->readnodefile = 1;
	...
	if (m->mesh_dim != 2) {
		printf("ERROR:  Triangle only works with two-dimensional "
		       "meshes.\n");
		triexit(1);
	}
	if (m->nextras == 0) {
	...
	for (i = 0; i < m->invertices; i++) {
		...
		if (nodemarkers) {
			...
			/* If no markers are specified in the file, they default
			 * to zero. */
			setvertexmark(vertexloop, 0);
		}
		...
		/*for (j=0;j<2+m->nextras;j++) {
		      printf(" %.17g",vertexloop[j]);
		}
		printf("\n"); */
	}
	if (m->readnodefile) {
	...
	*holes = (int)strtol(stringptr, &stringptr, 0);
	...
	if ((b->regionattrib || b->vararea) && !b->refine) {
		...
		*regions = (int)strtol(stringptr, &stringptr, 0);
		if (*regions > 0) {
			regionlist =
			    (REAL *)trimalloc(4 * *regions * (int)sizeof(REAL));
			*rlist = regionlist;
			index = 0;
			for (i = 0; i < *regions; i++) {
				stringptr =
				...
				stringptr = findfield(stringptr);
				if (*stringptr == '\0') {
					printf("Error:  Region %d has no x "
					       "coordinate.\n",
					       b->firstnumber + i);
					triexit(1);
				} else {
				...
				stringptr = findfield(stringptr);
				if (*stringptr == '\0') {
					printf("Error:  Region %d has no y "
					       "coordinate.\n",
					       b->firstnumber + i);
					triexit(1);
				} else {
				...
				if (*stringptr == '\0') {
					printf(
					    "Error:  Region %d has no region "
					    "attribute or area constraint.\n",
					    b->firstnumber + i);
					triexit(1);
				} else {
					regionlist[index++] =
...

{
	...
	if (*pointlist == (REAL *)NULL) {
		*pointlist =
		    (REAL *)trimalloc((int)(outvertices * 2 * sizeof(REAL)));
	}
	/* Allocate memory for output vertex attributes if necessary. */
	if ((m->nextras > 0) && (*pointattriblist == (REAL *)NULL)) {
		*pointattriblist = (REAL *)trimalloc(
		    (int)(outvertices * m->nextras * sizeof(REAL)));
	}
	/* Allocate memory for output vertex markers if necessary. */
	if (!b->nobound && (*pointmarkerlist == (int *)NULL)) {
		*pointmarkerlist =
		    (int *)trimalloc((int)(outvertices * sizeof(int)));
	}
	plist = *pointlist;
	...
	/* Number of vertices, number of dimensions, number of vertex
	 * attributes, */
	/* and number of boundary markers (zero or one). */
	/* In the -r switch is not active we write 3 more attributes (z,u,v)
	 * with */
	/* respect to the input nodes. */
	if (!b->refine) {
		fprintf(outfile, "%ld  %d  %d  %d\n", outvertices, m->mesh_dim,
			m->nextras + 3, 1 - b->nobound);
	} else {
		fprintf(outfile, "%ld  %d  %d  %d\n", outvertices, m->mesh_dim,
	...
	while (vertexloop != (vertex)NULL) {
		if (!b->jettison || (vertextype(vertexloop) != UNDEADVERTEX)) {
			...
			if (!b->nobound) {
				...
				pmlist[vertexnumber - b->firstnumber] =
			...
			if (!b->refine) {
				...
				YY = YY < Y1 ? YY : Y1;
				z = altitude(XX, YY);
				set_normal(XX, YY, &u, &v);
				X = X > 0 ? X : 0;
				X = X < 1 ? X : 1;
				Y = Y > 0 ? Y : 0;
				Y = Y < 1 ? Y : 1;
				/* Vertex number, x and y coordinates, altitude
				 * z and normal components u & v. */
				fprintf(
				    outfile,
				    "%4d  %.17g  %.17g  %.17g  %.17g  %.17g",
				    vertexnumber, X, Y, z, u, v);
				for (i = 0; i < m->nextras; i++) {
					/* Write an attribute. */
					fprintf(outfile, "  %.17g",
						vertexloop[i + 2]);
...

{
	...
	if (*trianglelist == (int *)NULL) {
		*trianglelist = (int *)trimalloc(
		    (int)(m->triangles.items *
			  ((b->order + 1) * (b->order + 2) / 2) * sizeof(int)));
	}
	...
	while (triangleloop.tri != (triangle *)NULL) {
		...
		if (b->order == 1) {
			...
			fprintf(outfile, "%4ld    %4d  %4d  %4d  %4d  %4d  %4d",
				elementnumber, vertexmark(p1), vertexmark(p2),
				vertexmark(p3), vertexmark(mid1),
...

{
	...
	while (subsegloop.ss != (subseg *)NULL) {
		...
		if (!b->nobound) {
			/* Copy the boundary marker. */
			smlist[subsegnumber - b->firstnumber] =
			    mark(subsegloop);
		}
#else  /* not TRILIBRARY */
		/* Segment number, indices of its two endpoints, and possibly a
		 * marker. */
		if (b->nobound) {
			...
			fprintf(outfile, "%4ld    %4d  %4d    %4d\n",
				subsegnumber, vertexmark(endpoint1),
				vertexmark(endpoint2), mark(subsegloop));
		}
#endif /* not TRILIBRARY */
	...
	if (regions > 0) {
		...
		for (regionnumber = 0; regionnumber < regions; regionnumber++) {
			/* Region number, x and y coordinates, attribute,
			 * maximum area. */
...

{
	...
	if (!b->nobound && (*edgemarkerlist == (int *)NULL)) {
		*edgemarkerlist =
		    (int *)trimalloc((int)(m->edges * sizeof(int)));
	}
	elist = *edgelist;
	...
	while (triangleloop.tri != (triangle *)NULL) {
		for (triangleloop.orient = 0; triangleloop.orient < 3;
			...
			if ((triangleloop.tri < trisym.tri) ||
				...
				if (b->nobound) {
#ifndef TRILIBRARY
					/* Edge number, indices of two
					 * endpoints. */
					fprintf(outfile, "%4ld   %d  %d\n",
						edgenumber, vertexmark(p1),
						vertexmark(p2));
#endif /* not TRILIBRARY */
				} else {
					/* Edge number, indices of two
					 * endpoints, and a boundary marker. */
					/*   If there's no subsegment, the
					 * boundary marker is zero.      */
					if (b->usesegments) {
						tspivot(triangleloop,
							checkmark);
						if (checkmark.ss ==
						    m->dummysub) {
#ifdef TRILIBRARY
...

{
	...
	if (*vpointlist == (REAL *)NULL) {
		*vpointlist = (REAL *)trimalloc(
		    (int)(m->triangles.items * 2 * sizeof(REAL)));
	}
	...
	*vpointmarkerlist = (int *)NULL;
	...
	while (triangleloop.tri != (triangle *)NULL) {
		...
		dest(triangleloop, tdest);
		apex(triangleloop, tapex);
		findcircumcenter(m, b, torg, tdest, tapex, circumcenter, &xi,
				 &eta, 0);
#ifdef TRILIBRARY
		/* X and y coordinates. */
		...
		for (i = 2; i < 2 + m->nextras; i++) {
			/* Interpolate the vertex attributes at the
			 * circumcenter. */
			palist[attribindex++] = torg[i] +
						xi * (tdest[i] - torg[i]) +
						eta * (tapex[i] - torg[i]);
		}
#else  /* not TRILIBRARY */
		/* Voronoi vertex number, x and y coordinates. */
		fprintf(outfile, "%4ld    %.17g  %.17g", vnodenumber,
			circumcenter[0], circumcenter[1]);
		for (i = 2; i < 2 + m->nextras; i++) {
			/* Interpolate the vertex attributes at the
			 * circumcenter. */
			fprintf(outfile, "  %.17g",
				torg[i] + xi * (tdest[i] - torg[i]) +
				    eta * (tapex[i] - torg[i]));
		}
		fprintf(outfile, "\n");
#endif /* not TRILIBRARY */
	...
	if (*vedgelist == (int *)NULL) {
		*vedgelist =
		    (int *)trimalloc((int)(m->edges * 2 * sizeof(int)));
	}
	*vedgemarkerlist = (int *)NULL;
	/* Allocate memory for output Voronoi norms if necessary. */
	if (*vnormlist == (REAL *)NULL) {
		*vnormlist =
		    (REAL *)trimalloc((int)(m->edges * 2 * sizeof(REAL)));
	}
	elist = *vedgelist;
	normlist = *vnormlist;
	coordindex = 0;
	...
	while (triangleloop.tri != (triangle *)NULL) {
		for (triangleloop.orient = 0; triangleloop.orient < 3;
			...
			if ((triangleloop.tri < trisym.tri) ||
			    (trisym.tri == m->dummytri)) {
				/* Find the number of this triangle (and Voronoi
				 * vertex). */
				...
				if (trisym.tri == m->dummytri) {
					...
					dest(triangleloop, tdest);
#ifdef TRILIBRARY
					/* Copy an infinite ray.  Index of one
					 * endpoint, and -1. */
					elist[coordindex] = p1;
					normlist[coordindex++] =
					    tdest[1] - torg[1];
					...
					normlist[coordindex++] =
					    torg[0] - tdest[0];
#else  /* not TRILIBRARY */
					/* Write an infinite ray.  Edge number,
					 * index of one endpoint, -1, */
					/*   and x and y coordinates of a vector
					 * representing the          */
					...
					/* Find the number of the adjacent
					 * triangle (and Voronoi vertex). */
					p2 = *(int *)(trisym.tri + 6);
					/* Finite edge.  Write indices of two
					 * endpoints. */
#ifdef TRILIBRARY
					elist[coordindex] = p1;
...

{
	...
	if (*neighborlist == (int *)NULL) {
		*neighborlist = (int *)trimalloc(
		    (int)(m->triangles.items * 3 * sizeof(int)));
	}
	nlist = *neighborlist;
	index = 0;
#else  /* not TRILIBRARY */
...

{
	...
	while (vertexloop != (vertex)NULL) {
		if (!b->jettison || (vertextype(vertexloop) != UNDEADVERTEX)) {
			/* The "0.0" is here because the OFF format uses 3D
			 * coordinates. */
	...
	while (triangleloop.tri != (triangle *)NULL) {
		...
		fprintf(outfile, " 3   %4d  %4d  %4d\n",
			...
			vertexmark(p2) - b->firstnumber,
			vertexmark(p3) - b->firstnumber);
		triangleloop.tri = triangletraverse(m);
	}
	finishfile(outfile, argc, argv);
}
...

{
	...
	while (triangleloop.tri != (triangle *)NULL) {
		...
		aspectindex = 0;
		while ((triaspect2 >
			ratiotable[aspectindex] * ratiotable[aspectindex]) &&
		       (aspectindex < 15)) {
			aspectindex++;
		}
		...
		for (i = 0; i < 3; i++) {
			...
			cossquare = dotproduct * dotproduct /
				    (edgelength[j] * edgelength[k]);
			tendegree = 8;
			for (ii = 7; ii >= 0; ii--) {
	...
	if (biggestangle >= 1.0) {
		...
		if (acutebiggest) {
			...
			biggestangle =
			    180.0 - degconst * acos(sqrt(biggestangle));
		}
	}
	...
	printf("   1.1547 - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d\n",
	       ...
	       aspecttable[8]);
	for (i = 1; i < 7; i++) {
		printf("   %6.6g - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  "
		       "%8d\n",
		       ratiotable[i - 1], ratiotable[i], aspecttable[i],
		       ratiotable[i + 7], ratiotable[i + 8],
		       aspecttable[i + 8]);
	}
	printf("   %6.6g - %-6.6g    :  %8d    | %6.6g -            :  %8d\n",
	       ratiotable[6], ratiotable[7], aspecttable[7], ratiotable[14],
	       aspecttable[15]);
	printf("   (Aspect ratio is longest edge divided by shortest "
	       "altitude)\n\n");
	...
	for (i = 0; i < 9; i++) {
		printf("     %3d - %3d degrees:  %8d    |    %3d - %3d "
		       "degrees:  %8d\n",
		       i * 10, i * 10 + 10, angletable[i], i * 10 + 90,
		       i * 10 + 100, angletable[i + 9]);
	}
	printf("\n");
}
...

{
	...
	if (b->verbose) {
		...
		if (m->badsubsegs.maxitems > 0) {
			printf("   Maximum number of encroached subsegments: "
			       "%ld\n",
		...
		if (m->flipstackers.maxitems > 0) {
			printf("   Maximum number of stacked triangle flips: "
			       "%ld\n",
		...
		if (m->circumcentercount > 0) {
			printf("   Number of triangle circumcenter "
			       "computations: %ld\n",
			       m->circumcentercount);
		}
		printf("\n");
...

{
	...
	printf("   Loading curv_tol geographic weights.\n");
	fflush(stdout);
	weight = (float *)malloc(sizeof(float) * 1001 * 1001);
	weight_file = fopen(b.weight_filename, "rb");
	...
	for (i = 0; i < 1001; i++) {
		for (j = 0; j < 1001; j++) {
			dummy = fread(&weight[1001 * i + j], 4, 1, weight_file);
		}
	}
	fclose(weight_file);
...
#else  /* not TRILIBRARY */
		readholes(&m, &b, polyfile, b.inpolyfilename, &holearray,
			  &m.holes, &regionarray, &m.regions);
#endif /* not TRILIBRARY */
		if (!b.refine) {
			/* Carve out holes and concavities. */
			carveholes(&m, &b, holearray, m.holes, regionarray,
				   m.regions);
		}
	} else {
		/* Without a PSLG, there can be no holes or regional attributes
		 */
		/*   or area constraints.  The following are set to zero to
		 * avoid */
	...
	if (b.quality && (m.triangles.items > 0)) {
		enforcequality(&m,
			       &b); /* Enforce angle and area constraints. */
	}
#endif /* not CDT_ONLY */
	...
	if (b.order > 1) {
		highorder(
		    &m, &b); /* Promote elements to higher polynomial order. */
	}
	if (!b.quiet) {
		/*printf("\n");*/
	}
	...
	if (b.nonodewritten || (b.noiterationnum && m.readnodefile)) {
		...
		numbernodes(&m,
			    &b); /* We must remember to number the vertices. */
	} else {
...
#ifdef TRILIBRARY
		writeelements(&m, &b, &out->trianglelist,
	...
	if (b.poly || b.convex) {
		/* If not using iteration numbers, don't overwrite the .poly
		 * file. */
...
#else  /* not TRILIBRARY */
			writepoly(&m, &b, b.outpolyfilename, holearray, m.holes,
				  regionarray, m.regions, argc, argv);
#endif /* not TRILIBRARY */
		}
	}
#ifndef TRILIBRARY
...
#else  /* not TRILIBRARY */
		writevoronoi(&m, &b, b.vnodefilename, b.vedgefilename, argc,
			     argv);
#endif /* not TRILIBRARY */
	}
	if (b.neighbors) {
