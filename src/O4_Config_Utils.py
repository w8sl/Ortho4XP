...
cfg_vars={
    ...
    # Mesh
    'curvature_tol':       {'type':list,'default':[2],'hint':"This parameter is intrinsically linked the mesh final density. Mesh refinement is mostly based on curvature computations on the elevation data (the exact decision rule can be found in _ triunsuitable() _ in Utils/Triangle4XP.c). A higher curvature tolerance yields fewer triangles."},
    'apt_curv_tol':        {'type':list,'default':[0.5],'hint':"If smaller, it supersedes curvature_tol over airports neighbourhoods."},
    'apt_curv_ext':        {'type':list,'default':[0.5],'hint':"Extent (in km) around the airports where apt_curv_tol applies."},
    'coast_curv_tol':      {'type':list,'default':[1],'hint':"If smaller, it supersedes curvature_tol along the coastline."},
    'coast_curv_ext':      {'type':list,'default':[0.5],'hint':"Extent (in km) around the coastline where coast_curv_tol applies."},
    'limit_tris'    :      {'type':int,  'default':0,'hint':"If non zero, upper bound on the number of final triangles in the mesh."},
    'hmin':                {'type':list,'default':[0],'hint':"The mesh algorithm will not try to subdivide triangles whose shortest edge is already smaller than hmin (in meters). If hmin is smaller than half of the levation data step size, it will default to it anyhow (its default zero value thus means : as good as the DEM can do)."}, 
    'min_angle':           {'type':list,'default':[10],'hint':"The mesh algorithm will try to not have mesh triangles with second smallest angle less than the value (in deg) of min_angle (prior to v1.3 it was the smallest, not second smallest) The goal behind this is to avoid potential artifacts when a triangle vertex is very close the the middle of its facing edge."},
...
try:
    ...
    for line in f.readlines():
        ...
        try:
            ...
            target=cfg_vars[var]['module']+"."+var if 'module' in cfg_vars[var] else var
            if cfg_vars[var]['type']==list and value and value[0]!='[': value='['+value+']'
    ...
    def apply_changes(self):
        ...
        for var in list_tile_vars+list_app_vars:
            try:
                ...
                if cfg_vars[var]['type']==list and value and value[0]!='[':
                    value='['+value+']'
                    self.v_[var].set(value)
                if cfg_vars[var]['type'] in (bool,list):
