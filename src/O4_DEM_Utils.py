...
available_sources = (
    ...
    "View",
    'Viewfinderpanoramas (J. de Ferranti) 1" (USA,CA,Greenland,EU,NZ) or 3" - mostly worldwide',
    "SRTM",
    "SRTMv3 (from OpenTopography) - worldwide at latitudes < 60",
    "NED1",
    'NED 1" (from USGS) - USA, Canada, Mexico',
    "NED1/3",
    'NED 1/3" (from USGS) - USA',
    "ALOS",
    "ALOS 3W30 average (manually downloaded from www.eorc.jaxa.jp) - mostly worldwide",
    "SRTMv3",
    'SRTMv3 1" GeoTiffs (manually downloaded from USGS)',
)
...
class DEM:
    ...
    def super_level_set(self, level, wgs84_bbox):
        ...
        return (
            (xmin + self.lon, xmax + self.lon, ymin + self.lat, ymax + self.lat),
    ...
    def alt_nostrict(self, node):
        ...
        t2 = self.alt_dem[
            (Nminusny - 1) * (Nminusny >= 1), (nx + 1) * (nx < Nx) + Nx * (nx == Nx)
        ]
...
def ensure_elevation(source, lat, lon, verbose=True):
    if source in ("View", "View3"):
        ...
        if os.path.exists(FNAMES.elevation_data(source, lat, lon)):
            ...
            return 1
        UI.vprint(
            1, "    Downloading ", FNAMES.elevation_data(source, lat, lon), "from USGS."
        )
        url_base = (
        ...
        if os.path.exists(FNAMES.elevation_data(source, lat, lon)):
            ...
            UI.vprint(
                1,
                "   SORRY: Please download "
                + FNAMES.elevation_data(source, lat, lon)
                + " from EarthExplorer first...",
            )
            UI.red_flag = True
            return 0
    else:
    ...
    while True:
        try:
            ...
            if r.status_code == requests.codes.ok:
                ...
                if verbose:
                    UI.vprint(2, f"    Server said '404 Not Found'")
                return 0
            else:
                if verbose:
                    UI.vprint(2, r.status_code)
