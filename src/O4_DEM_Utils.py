...
class DEM():
    def __init__(self,lat,lon,source='',fill_nodata=True,info_only=False):
        ...
        UI.vprint(1,"    * Min altitude:",self.alt_dem[self.alt_dem!=self.nodata].min(),", Max altitude:",self.alt_dem[self.alt_dem!=self.nodata].max(),", Mean:",self.alt_dem[self.alt_dem!=self.nodata].mean())
    ...
    def create_normal_map(self):
        pixx = GEO.lon_to_m(self.lat+0.5)/(self.nxdem-1)*(self.x1-self.x0)
        pixy = GEO.lat_to_m/(self.nydem-1)*(self.y1-self.y0)
        dx=numpy.zeros((self.nydem,self.nxdem),dtype=numpy.float32)
        dy=numpy.zeros((self.nydem,self.nxdem),dtype=numpy.float32)
        ...
        del(dy)
        band_b=Image.fromarray((numpy.ones((self.nydem,self.nxdem))*10).astype(numpy.uint8))
        band_a=Image.fromarray((numpy.ones((self.nydem,self.nxdem))*10).astype(numpy.uint8))
        self.normal_map=Image.merge('RGBA',(band_r,band_g,band_b,band_a))
        del(band_r)
        del(band_g)
        del(band_b)
        del(band_a)
    ...
    def alt_nostrict(self,node):
        ...
        return (1-rx)*(1-ry)*t1+rx*ry*t2+rx*(1-ry)*t3+(1-rx)*ry*t4 
    ...
    def alt_strict(self,node):
        ...
        x=node[0]
        y=node[1]
        if (x>self.x1) or (x<self.x0) or (y<self.y0) or (y>self.y1): return self.nodata
        px=(x-self.x0)/(self.x1-self.x0)*Nx
        py=(y-self.y0)/(self.y1-self.y0)*Ny
        nx=int(px)
        Nminusny=Ny-int(py)
        ...
        t3=self.alt_dem[Nminusny,(nx+1)*(nx<Nx)+Nx*(nx==Nx)]
        t4=self.alt_dem[(Nminusny-1)*(Nminusny>=1),nx]
        if t1==self.nodata or t2==self.nodata or t3==self.nodata or t4==self.nodata: return self.nodata
        return (1-rx)*(1-ry)*t1+rx*ry*t2+rx*(1-ry)*t3+(1-rx)*ry*t4 
        #WAS:
        #x=node[0]
        #y=node[1]
    ...
    def alt_vec_strict(self,way):
        ...
        Ny=self.nydem-1
        x,y=way[:,0],way[:,1]
        out=self.nodata*numpy.ones(x.shape,dtype=numpy.float32)
        ...
        rx=px-nx
        ry=py+Nminusny-Ny
        t1=numpy.array([self.alt_dem[i][j] for i,j in zip(Nminusny,nx)])
        mask*=(t1!=self.nodata)
        t2=numpy.array([self.alt_dem[i][j] for i,j in zip((Nminusny-1)*(Nminusny>=1),(nx+1)*(nx<Nx)+Nx*(nx==Nx))])
        mask*=(t2!=self.nodata)
        t3=numpy.array([self.alt_dem[i][j] for i,j in zip(Nminusny,(nx+1)*(nx<Nx)+Nx*(nx==Nx))])
        mask*=(t3!=self.nodata)
        t4=numpy.array([self.alt_dem[i][j] for i,j in zip((Nminusny-1)*(Nminusny>=1),nx)])
        mask*=(t4!=self.nodata)
        out[mask]=((1-rx)*(1-ry)*t1+rx*ry*t2+rx*(1-ry)*t3+(1-rx)*ry*t4)[mask]
        return out
        #WAS
        #x,y=way[:,0],way[:,1]
        #mask=(x>=self.x0)*(x<=self.x1)*(y>=self.y0)*(y<=self.y1)
...
##############################################################################
def smoothen(raster,pix_width,mask_array,preserve_boundary=True):
    if not pix_width: return raster
    if not mask_array.max(): return raster
