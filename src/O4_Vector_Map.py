...
def include_airports(vector_map,tile): 
    ...
    APT.build_building_areas(tile,airport_layer,dico_airports)
    ...
    APT.smooth_raster_over_airports(tile.dem,tile.apt_smoothing_pix[0],dico_airports,FNAMES.alt_file(tile))
    iterate=1
    if hasattr(tile.dem,'subdems'):
        for dem in tile.dem.subdems:
            apt_smoothing_pix=tile.apt_smoothing_pix[iterate] if len(tile.apt_smoothing_pix)>iterate else tile.apt_smoothing_pix[0]
            APT.smooth_raster_over_airports(dem,apt_smoothing_pix,dico_airports,FNAMES.alt_file(tile,iterate),preserve_boundary=False)
            iterate+=1
    del(iterate)
