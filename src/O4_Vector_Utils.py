...
class Vector_Map():
    ...
    dico_attributes = {'DUMMY':0,'WATER':1,'SEA':2,'SEA_EQUIV':4,'INTERP_ALT':8,'RUNWAY':16,'TAXIWAY':32,'APRON':64,'BUILDING':8}
    def __init__(self):
    ...
    def insert_edge(self,id0,id1,marker,check=True):
        ...
        for hits in task: 
            ...
            coeffs=are_encroached(numpy.array(self.nodes_dico[id0]),\
                    numpy.array(self.nodes_dico[id1]),\
                    numpy.array(self.nodes_dico[id2]),\
                    numpy.array(self.nodes_dico[id3]))
            if not coeffs[0]: 
                continue
            if coeffs[0] == 2: #transverse encroachment 
                (alpha,beta)=coeffs[1][:2]   
                if beta not in (0,1):
                ...
                (alpha0,alpha1,beta0,beta1)=coeffs[1]
...
def are_encroached(a,b,c,d): 
        ...
        if abs(numpy.linalg.det(A))>eps:
            ...
            return (alpha>=0 and alpha<=1) and (beta>=0 and beta<=1) and ((alpha>enc_lim and alpha<1-enc_lim)\
                    or (beta>enc_lim and beta<1-enc_lim)) and (2,(alpha,beta,0.0,0.0)) or (0,(0.0,0.0,0.0,0.0)) 
        elif abs(numpy.linalg.det(numpy.column_stack((b-a,c-a))))>eps:
            return (0,(0.0,0.0,0.0,0.0))    
        else:
            g_idx = numpy.argmax(numpy.abs(a-b))
            d_idx = numpy.argmax(numpy.abs(c-d))
            alpha0,alpha1=(a-c)[g_idx]/(a-b)[g_idx],(a-d)[g_idx]/(a-b)[g_idx]
            beta0,beta1=(c-a)[d_idx]/(c-d)[d_idx],(c-b)[d_idx]/(c-d)[d_idx]
            return (alpha0>0 or alpha1>0) and (alpha0<1 or alpha1<1) and (4,(alpha0,alpha1,beta0,beta1)) or (0,(0.0,0.0,0.0,0.0))
...
def split_polygon(input_pol, max_size,count=0):
    ...
    if xmax-xmin >= ymax-ymin:
        ...
        subpols1 = input_pol.intersection(geometry.box(xmin - 1, ymin - 1, xcut, ymax + 1))
        #subpols2 = input_pol.difference(subpols1)
        subpols2 = input_pol.intersection(geometry.box(xcut, ymin - 1, xmax + 1, ymax + 1))
    else:
        ycut=numpy.round((ymin+ymax)/2,6)
        subpols1 = input_pol.intersection(geometry.box(xmin - 1, ymin - 1, xmax + 1, ycut))
        #subpols2 = input_pol.difference(subpols1)
        subpols2 = input_pol.intersection(geometry.box(xmin - 1, ycut, xmax + 1, ymax + 1))
    tmp_val=[]
...
##############################################################################
##############################################################################
def MultiPolygon_to_Indexed_Polygons(multipol,merge_overlappings=True,idx_pol=None,dico_pol=None,id_pol=0):
    ########################################################################
    def merge_pol(pol,id_pol):
        ids_to_merge=[]
        for polid in idx_pol.intersection(pol.bounds):
            if pol.intersection(dico_pol[polid]).area or (merge_overlappings == "touch" and pol.intersection(dico_pol[polid])):
        ...
        #for pol in merged_pols.geoms if 'Multi' in merged_pols.geom_type else [merged_pols]:
            ...
            #for subpol in [pol]: 
        for subpol in split_polygon(merged_pols,0.07,0):
    ...
    ########################################################################
    UI.progress_bar(1,0)
    if not idx_pol: idx_pol=index.Index() 
...
def weighted_alt(node,alt_idx,alt_dico,dem):
    ...
    for  idx in alt_idx.intersection((x-eps1,y-eps1,x+eps1,y+eps1)):
        (geom,fit,width)=alt_dico[idx]
        if isinstance(geom,geometry.LineString):
            dist=pt.distance(geom)*GEO.lat_to_m 
            weight=numpy.exp(-dist/(2*width))
            # here fit is a numpy.polyfit output 
            alti+=numpy.polyval(fit,geom.project(pt,normalized=True))*weight
        elif isinstance(geom,geometry.Polygon):
            dist=pt.distance(geom)*GEO.lat_to_m 
            weight=numpy.exp(-dist/(2*width))
            # here fit is a float representing the mean dem altitude of the polygon (typically building footprint) 
            alti+=fit*weight
        else:
            UI.vprint(1,"ERROR: surfaces of type",geom.geom_type,"are not valid types for auto-patching.")
            UI.bug_report()
            continue
        weights+=weight
    if weights<1e-6:
