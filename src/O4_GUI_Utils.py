...
class Ortho4XP_Custom_ZL(tk.Toplevel):
    ...
    dico_color= {15:'cyan',16:'green',17:'yellow',18:'orange',19:'red',20:'red',21:'red'}
    zl_list   = ['10','11','12','13']
    points=[]
    coords=[]
    polygon_list=[]
    ...
    def __init__(self,parent,lat,lon):
        ...
        self.extract_mesh_btn.grid(row=row, column=0, padx=5, pady=3, sticky=N+S+E+W)
        row+=1 
    ...
    def show_tile_preview(self,filepreview,lat,lon):
        ...
        self.extract_mesh_btn.bind("<Shift-ButtonPress-1>", self.reintroduce_mesh_ifc) 
        self.polygon_list=[]
        self.polyobj_list=[]
        self.poly_curr=[]
        bdpoints=[]
    ...
    def reintroduce_mesh_ifc(self, event):
        ...
        build_dir = FNAMES.build_dir(self.lat, self.lon, self.parent.custom_build_dir.get())
        mesh_file = FNAMES.mesh_file(build_dir, self.lat, self.lon)
        UI.vprint(1,"Reintroduce part of ",mesh_file,"from",FNAMES.obj_file(til_x_left,til_y_top,zoomlevel,provider_code)) 
