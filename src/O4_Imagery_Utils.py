        ...
        for rlayer in local_combined_providers_dict[provider_code]:
            ...
            if len(local_combined_providers_dict[provider_code])==1 or has_data((x0,y0,x1,y1),rlayer['extent_code'],is_mask_layer= (tile.lat,tile.lon, tile.mask_zl) if rlayer['priority']=='mask' else False):
                ...
                if not os.path.isfile(os.path.join(true_file_dir,true_file_name)):
                    ...
                    UI.vprint(2,"   The orthophoto "+true_file_name+" (for combining in "+provider_code+") is already present.")
        if not data_found: 
...
###############################################################################################################################
def geotag_old(input_file_name):
    suffix=input_file_name.split('.')[-1]
    out_file_name=input_file_name.replace(suffix,'tiff')
    items=input_file_name.split('_')
    til_y_top=int(items[0])
...
###############################################################################################################################
def HSV_value(lat, lon, provider_code, zoomlevel, cached_image = None):
    (til_x_left, til_y_top) =  GEO.wgs84_to_orthogrid(lat, lon, zoomlevel)
    (s, t) = GEO.st_coord(lat, lon, til_x_left, til_y_top, zoomlevel)
    if not cached_image:
        file_name = FNAMES.jpeg_file_name_from_attributes(til_x_left, til_y_top, zoomlevel, provider_code)
        file_dir=FNAMES.jpeg_file_dir_from_attributes(int(floor(lat)), int(floor(lon)), zoomlevel, providers_dict[provider_code])
        print(os.path.join(file_dir,file_name))
        if not os.path.exists(os.path.join(file_dir,file_name)):
            download_jpeg_ortho(file_dir, file_name, til_x_left, til_y_top, zoomlevel, provider_code, super_resol_factor=1)
        cached_image = Image.open(os.path.join(file_dir,file_name)) 
    (r,g,b) = cached_image.getpixel((4096*s,4096*(1-t)))  
    max = r
    min = r
    if g >= max:
        max = g
    ...
    S = 0 if max == 0 else 1 - min/max
    V = max/255
    return (int(H),int(100*S),int(100*V))   
###############################################################################################################################            
