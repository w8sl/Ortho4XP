...
if "dar" in sys.platform:
    dds_convert_cmd = os.path.join(UI.Ortho4XP_dir, "Utils", "nvcompress", "new", "mac",  "nvcompress")
            ...
            for line in f.readlines():
                ...
                if key == "mask_bounds":
                    ...
                    try:
                        ...
                        print(
                            "Error in reading mask buffer width for extent", extent_code
                        )
                        valid_extent = False
                elif key == "mask_width":
                    try:
            ...
            for line in f.readlines():
                ...
                if key == "grid_type" and value not in ["webmercator"]:
                    ...
                    if len(value.split(".")) < 2:
                        print(
                            "Error in reading wms version for provider", provider_code
                        )
                        valid_provider = False
                    ...
                    try:
                        ...
                        print(
                            "Error in reading resolutions for provider", provider_code
                        )
                        valid_provider = False
            ...
            if "request_type" in provider and provider["request_type"] == "wmts":
                try:
                    ...
                    try:
                        ...
                        print(
                            "Error in reading capabilities for provider", provider_code
                        )
                        valid_provider = False
                if valid_provider:
                    try:
        ...
        try:
            ...
            if ("[200]" in status_code) and ("image" in r.headers["Content-Type"]):
                try:
                    ...
                    UI.vprint(
                        2, "Server said 'OK', but the received image was corrupted."
                    )
                    UI.vprint(3, url, r.headers)
    ...
    while True:
        ...
        if has_URL and provider["code"] in URL.custom_url_list:
            ...
            if os.path.isfile(url_local):
                ...
                UI.vprint(
                    2, "! File ", url_local, "absent, using white texture instead !"
                )
                return (
                    0,
                    Image.new(
                        "RGB", (provider["tile_size"], provider["tile_size"]), "white"
                    ),
                )
        if not request_headers:
            if "fake_headers" in provider:
...
def get_and_paste_wmts_part(
    tilematrix, til_x, til_y, provider, big_image, x0, y0, http_session, subt_size=None
):
...
def build_texture_from_bbox_and_size(t_bbox, t_epsg, t_size, provider):
    ...
    if provider["request_type"] == "wms":
        ...
        wmts_tilematrix = (
            numpy.abs(provider["resolutions"] - asked_resol * 1.1)
        ).argmin()
        wmts_resol = provider["resolutions"][
            wmts_tilematrix
...
def download_jpeg_ortho(
    ...
    if not success:
        UI.lvprint(
            ...
            "Part of image",
            file_name,
            "could not be obtained (even at lower ZL), it was filled with white there.",
        )
        if not os.path.exists(file_dir):
            os.makedirs(file_dir)
        # This will create an empty file with the .white extension
        with open(
            os.path.join(file_dir, file_name.replace(".jpg", ".white")), "w"
        ) as file:
            pass
    ...
    try:
        ...
        UI.lvprint(
            0, "OS Error : could not save orthophoto on disk, received message :", e
        )
        return 0
    return 1
...
def build_jpeg_ortho(
    ...
    if provider_code in local_combined_providers_dict:
        ...
        for rlayer in local_combined_providers_dict[provider_code]:
            ...
            if len(local_combined_providers_dict[provider_code]) == 1 or has_data(
                (x0, y0, x1, y1),
                rlayer["extent_code"],
                is_mask_layer=(
                    (tile.lat, tile.lon, tile.mask_zl)
        ...
        if not data_found:
            UI.lvprint(
                1,
                "     -> !!! Warning : No data found for building the combined texture",
        ...
        if out_file_name:
        ...
        ):  # In case one would like to save combined orthos as jpegs (this can be useful to use different masks parameters for imagery masks layers and actual masks
            ...
            try:
                ...
                UI.lvprint(
                    0,
                    "OS Error : could not save orthophoto on disk, received message :",
                    e,
                )
                return 0
...
def color_transform(im, color_code):
    try:
        for color_filter in color_filters_dict[color_code]:
            if (
                color_filter[0] == "brightness-contrast"
            ):  # both range from -127 to 127, http://gimp.sourcearchive.com/documentation/2.6.1/gimpbrightnesscontrastconfig_8c-source.html
                ...
                color_filter[0] == "levels"
            ):  # levels range between 0 and 255, gamma is neutral at 1 / https://pippin.gimp.org/image-processing/chap_point.html
                bands = im.split()
                for j in [0, 1, 2]:
...
def combine_textures(tile, til_x_left, til_y_top, zoomlevel, provider_code):
    ...
    mask_weight_below = numpy.zeros((4096, 4096), dtype=numpy.uint16)
    if (
        len(local_combined_providers_dict[provider_code]) == 1
...
def r_warp(filein, fileout, r_crs):
    ...
    with rasterio.open(filein) as src:
        ...
        new_transform = transform * transform.scale((width / 4096), (height / 4096))
        kwargs = src.meta.copy()
        kwargs.update(
            {
                "crs": r_crs,
        ...
        data = src.read(
            out_shape=(src.count, 4096, 4096), resampling=Resampling.bilinear
        )
